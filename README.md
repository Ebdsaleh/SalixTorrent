...
**Current `dev` extraction — transactional preview ownership:** Tranche 12 is published at `8d05ca8b059cef0ba386f324c215f2e80a9bfa83`. Tranche 13 is now real-Windows accepted and committed locally at `ddfb3ef2269acb33484cd6fe2f99af47fa40140f`: the optional designer-preview host owns one reconstructed preview, prepares replacement trees transactionally, disposes the previous rendered tree only after a candidate succeeds, and couples property/structural edits plus undo/redo to preview replacement without teaching the core component/runtime layer about a designer. The repaired Windows gate passed **655 / 655** in both complete discovery forms with one expected skip, plus 11 / 11 preview-host, 67 / 67 designer/relocation, 64 / 64 GUI-component and 23 / 23 Tkinter gates; manual Dear PyGui/Tkinter/SalixTorrent smoke and pre-commit also passed. Code-first Python composition and future designer-authored documents remain equal construction paths over the same semantic framework. Publication is paused only for this post-acceptance documentation closure; incremental toolkit mutation, callback/service serialization, specialized semantic-field reconstruction, project persistence, copy/paste/drag/drop tooling and final ecosystem naming/API freeze remain deferred.
