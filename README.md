...
**Current `dev` extraction — post-v0.5.1 designer component-palette request surface:** Tranche 4 is published at `31d0d4207c12ffd226bddd29a2a81d26c92e98dd` (`Add designer preview click-selection surface`). Tranche 5 adds `DesignerComponentPalette`, a renderer-neutral catalog/toolbox presenter that exposes framework component metadata on Dear PyGui and Tkinter but returns explicit `DesignerComponentInsertRequest` values instead of guessing parent/slot/relationship placement or mutating the document. The designer-shell proof now presents Components + Hierarchy beside the existing Preview + Inspector workflow. This tranche also fixes `View -> Torrent Details` menu routing by carrying each semantic tab key as Dear PyGui `user_data`, preventing empty-tab callback dispatch. Preparation passes 14 / 14 palette tests, 6 / 6 structural/menu-routing tests, 232 / 232 designer/relocation tests, 64 / 64 GUI-component tests, 39 / 39 Tkinter tests and **837 / 837** complete discovery. The full-commit Windows validator, visual smoke and `pre_commit_check.bat` remain authoritative before committing.**
