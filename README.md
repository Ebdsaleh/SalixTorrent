...
**Current `dev` extraction — designer selection/focus state:** Tranche 14 copy/paste/duplicate is accepted and published at `f59a9a392bf64820f59787b9449d6b7ca9b3e634`. Tranche 15 stable-ID selection/focus is now Windows-accepted and locally committed as `98f64e2` (`Add stable designer selection and focus state`), with publication pending for the documentation closure. One selected node ID and one focused node ID are tracked independently from document/history state, retained across property edits, moves, reparenting and whole-preview replacement, and reconciled to the nearest surviving ancestor when a selected/focused subtree is removed. Selection/focus never stores toolkit objects, dirties a project, creates undo history or forces a preview rebuild; `DesignerPreviewHost` resolves the current selected/focused IDs against whichever reconstructed preview generation it currently owns. Real-Windows acceptance passed 12 / 12 selection tests, 91 / 91 designer/relocation tests, 64 / 64 GUI-component tests, 25 / 25 Tkinter tests and both complete **681 / 681** discovery forms with one expected skip; localization remained current and manual visual/pre-commit gates passed. The code-first path remains equal and independent: ordinary applications compose the same semantic `Component` tree directly and never need designer modules. Multi-selection, visual overlays, hit-testing, pointer drag/drop/resize gestures, project persistence/schema and final ecosystem naming/API freeze remain deferred.
