...
> **Release status:** this source is the **v0.5.1 release checkpoint**. It packages the accepted post-v0.5.0 backend-neutral ecosystem extraction through Tranche 21, with the current real-Windows baseline at **759 / 759** tests in both complete discovery forms and one expected non-Windows shell-behavior skip. The annotated `v0.5.1` tag should point at the accepted release commit containing this exact version metadata. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` should advance only by an explicit release decision.
...
**Current `dev` extraction — designer editor-shell commands:** Tranche 20 workspace coordination is published at `f3a96420a6267089198f62662ab0761475ad962a`. Tranche 21 adds backend-neutral `DesignerShellCommands` / `DesignerShellCommandState` over the existing `DesignerWorkspace`, projecting stable File/Edit/Navigate command trees, dynamic Undo/Redo labels, selection-dependent Copy/Duplicate/Paste/Reveal availability and stable-ID hierarchy navigation without giving a concrete toolkit authority over editor semantics. Direct Save, Undo/Redo, Copy, Duplicate, Reveal and navigation delegate to the existing owners; New/Open/Save-As and Paste return explicit shell requests instead of guessing file-dialog, template or placement policy. Real-Windows acceptance passes 12 / 12 shell-command tests, 163 / 163 designer/relocation tests, 64 / 64 GUI-component tests, 31 / 31 Tkinter tests and both **759 / 759** complete discovery forms with one expected skip. Localization remains 1,337/current, headless reports 3 updates, compileall/Git checks pass, the manual visual smoke passed, and `pre_commit_check.bat` passes. This remains optional RAD tooling: ordinary code-first applications continue constructing the same semantic `Component` trees directly. Tranche 21 completes the backend-neutral designer prerequisite sequence for the v0.5.1 checkpoint; concrete editor surfaces remain deliberately deferred beyond this release boundary.**
