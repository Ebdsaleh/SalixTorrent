...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` has published post-v0.5.1 designer-surface Tranches 1–9, most recently Tranche 9 at `1870e053c444e2ca390543cb62a721b162f5ff61` (`Add designer pointer resize surface`). The current accepted real-Windows baseline is **905 / 905** in both Tranche-9 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` advances only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 designer numeric manipulation:** Tranche 9 is published at `1870e053c444e2ca390543cb62a721b162f5ff61` after real-Windows acceptance of the repaired selected-component resize handle. Tranche 10 adds shared numeric drag translation, Inspector `<>` scrub handles, one-click Reset all to defaults, normal/Shift/Ctrl sensitivity for both scrubbing and physical resize, and replaces Tkinter's native Sizegrip with a darker designer-owned child handle that cannot resize the outer application window. Reset is one checked composite history step and only clears properties whose metadata already defines portable Default/unset semantics; required content is preserved. Preparation targets **921 / 921** complete discovery with a 33-test reset/scrub gate, 19 pointer-resize regressions and 311 designer/relocation tests; real-Windows DPG interaction remains authoritative before commit.**
