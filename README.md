...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` has published post-v0.5.1 designer-surface Tranches 1–5, most recently Tranche 5 at `0b821dd1ec9e7b59ba6838ca8ede2dfdd4e1e323` (`Add designer component palette and fix detail menu`). The current accepted real-Windows baseline is **837 / 837** in both Tranche-5 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` advances only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 selected-node structural commands/shortcuts:** Tranche 6 is published at `9b61c6f61c8c7eb451a52b12283873d8a96555c0`. Tranche 7 extends `DesignerShellCommands` with checked Move Up / Move Down / Remove operations for the current non-root selection and adds backend-neutral `DesignerShellShortcuts` with concrete Dear PyGui/Tkinter key adapters (`Ctrl+D`, `Ctrl+Delete`, `Alt+Up`, `Alt+Down`). The commands reuse existing stable-ID structural/history/preview ownership, so reordering preserves selection and removal uses the established selection fallback/undo semantics. This deliberately gives a familiar keyboard/menu proof of placement and ordering before drag/drop is introduced. The designer-shell example also uses a framework-owned vertical split for Components / Placement / Hierarchy so all three left-side surfaces remain visible and independently bounded on Dear PyGui as well as Tkinter; the Dear PyGui Placement form uses explicit left-hand caption rows with conservative fixed-width editors so neither native trailing-label geometry nor table-cell fill sizing can push those fields outside the bounded pane. Preparation passes 16 / 16 focused structural-shortcut tests, 264 / 264 designer/relocation tests, 64 / 64 GUI-component tests and **871 / 871** complete discovery; the full-commit real-Windows validator, 41 / 41 Tkinter gate, visual smoke and `pre_commit_check.bat` remain authoritative before committing.**
