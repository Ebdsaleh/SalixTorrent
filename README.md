...
**Current `dev` extraction — designer hierarchy navigation:** Tranche 16 project-file ownership/save-load is published at `d95f6a1987aaff1bda9fb270c6219b36b8ee2080`. Tranche 17 adds backend-neutral hierarchy traversal over stable designer node IDs: parent, first/last child, previous/next sibling, previous/next preorder, plus root-to-parent reveal paths for future hierarchy surfaces. Navigation and reveal state stay ephemeral, do not dirty project documents or touch undo/redo, and do not rebuild previews; selection and focus can navigate independently or be coupled explicitly. A real Tkinter proof navigates the rendered blank-application hierarchy by stable ID while preview generation remains unchanged. Real-Windows acceptance passed **707 / 707** in both complete discovery forms with one expected skip, plus 12 / 12 navigation tests, 115 / 115 combined designer/relocation tests, 64 / 64 GUI-component tests and 27 / 27 Tkinter tests; localization remained current, headless/compileall/Git checks passed, the manual visual smoke was completed before automated validation, and `pre_commit_check.bat` passed. The accepted implementation is `34b2084` (`Add designer hierarchy navigation`), with publication pending the documentation closure and push. This remains optional designer tooling: ordinary code-first applications continue composing semantic `Component` trees directly and never require designer navigation/project modules. Toolkit tree widgets, persistent expansion state, keyboard binding policy, multi-selection, pointer hit-testing, drag/drop/resize handles and final public API/package naming remain deferred.**
