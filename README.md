...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` has published post-v0.5.1 designer-surface Tranches 1–11, most recently Tranche 11 at `7bc5192ccbd486483967351e016a066d3e94d9ce` (`Add designer layout autonomy and hierarchy reparenting`). The current accepted real-Windows baseline is **946 / 946** in both Tranche-11 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` advances only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 live Inspector scrub preview:** Tranche 11 is published at `7bc5192ccbd486483967351e016a066d3e94d9ce` after real-Windows acceptance of independent sibling geometry plus conservative hierarchy drag/reparent. Tranche 12 keeps the existing `<>` numeric scrub interaction but makes its effect visible in Preview while the mouse is still held: each drag sample reconstructs an ephemeral candidate from the authoritative session snapshot, refreshes the selected preview binding, and never touches document/history/dirty state. Mouse release still performs exactly one checked semantic property edit, so Undo remains one-step. Preparation targets **956 / 956** complete discovery with a 10-test Tranche-12 focused gate, 344 designer/relocation tests and 46 Tkinter tests; real-Windows Dear PyGui visual responsiveness remains authoritative before commit.**
