...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` has published post-v0.5.1 designer-surface Tranches 1–8, most recently Tranche 8 at `44745b0df1c9e41394ed7ab27674815cfe6d1ea3` (`Add designer preview sizing and resize controls`). The current accepted real-Windows baseline is **885 / 885** in both Tranche-8 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` advances only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 designer pointer resize surface:** Tranche 8 is published at `44745b0df1c9e41394ed7ab27674815cfe6d1ea3`. Tranche 9 adds the first direct-manipulation resize bridge: a selected rendered component receives a transient bottom-right resize handle, concrete Dear PyGui/Tkinter hosts own pointer mechanics and live drag feedback, and mouse release commits width/height once through the same checked `DesignerWorkspace` property/history/preview transaction already proven by Inspector. Real-Windows visual testing caught a Dear PyGui-only arbitration bug before publication: the original handle straddled the component boundary and could let the parent/root selection handler steal the gesture. The repaired DPG path keeps the handle fully inside the selected control, shares transient pointer ownership between selection and resize adapters, blocks normal selection on a handle press, and freezes the original stable ID/native item until release. The narrow Tkinter Inspector also keeps Apply/None/Default actions beneath editors. Preparation now passes 18 / 18 pointer-resize tests, 295 / 295 designer/relocation tests, 64 / 64 GUI-component tests, 44 / 44 live Tkinter tests and **905 / 905** complete discovery; real-Windows direct-manipulation visual acceptance and `pre_commit_check.bat` remain authoritative before commit.**
