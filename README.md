...
> **Release status:** this source is the **v0.5.1 release checkpoint**. It packages the accepted post-v0.5.0 backend-neutral ecosystem extraction through Tranche 21, with the current real-Windows baseline at **759 / 759** tests in both complete discovery forms and one expected non-Windows shell-behavior skip. The annotated `v0.5.1` tag points to release commit `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` should advance only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 designer command-menu surface:** the `v0.5.1` tag peels to release commit `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`; the final pre-release semantic tranche is `d16e1936990093d1250dc01d1aa7128c9b26f73f` (`Add designer editor-shell commands`). Post-v0.5.1 Tranche 1 adds `DesignerShellMenu`, the first concrete presentation bridge over that semantic core. It binds the existing `DesignerShellCommands` tree to the already renderer-neutral `CommandMenuHost`, refreshing dynamic command state before presentation and revalidating dispatch without importing Dear PyGui/Tkinter into the framework layer. A new `examples/ecosystem_designer_shell.py` proof renders the same semantic preview and command surface on both supported desktop backends. Real-Windows acceptance passes 12 / 12 command-surface tests, 175 / 175 designer/relocation tests, 64 / 64 GUI-component tests, 32 / 32 Tkinter tests and both **772 / 772** complete discovery forms with one expected skip; localization remains 1,337/current, headless reports 3 updates, compileall/Git and manual visual checks pass, and `pre_commit_check.bat` passes. New/Open/Save-As/Paste placement remain explicit shell requests; hierarchy widgets, property editors, file dialogs, pointer-driven editing and final public API/schema naming remain later work.**
