...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` continues with post-v0.5.1 designer-surface tranches; Tranche 1 is published at `376ae76409bbdc47bda0c06e49ba8e3bed920203`, and Tranche 2 is real-Windows accepted with the current baseline at **786 / 786** in both complete discovery forms and one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` should advance only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 designer hierarchy-panel surface:** Tranche 1 is published at `376ae76409bbdc47bda0c06e49ba8e3bed920203` (`Add designer command-menu surface`) over the `v0.5.1` release. Tranche 2 adds `DesignerHierarchyPanel`, a renderer-neutral presentation coordinator over the accepted `DesignerHierarchyRow` projection, plus explicit Dear PyGui and Tkinter hierarchy-panel hosts. Selection/focus, expansion and reveal remain owned by `DesignerWorkspace`; toolkit rows are disposable bindings and do not become designer state. `examples/ecosystem_designer_shell.py` now presents a live hierarchy beside the reconstructed preview while retaining the accepted Designer Commands surface on both desktop backends. Real-Windows acceptance passes 12 / 12 hierarchy-panel tests, 187 / 187 designer/relocation tests, 64 / 64 GUI-component tests, 34 / 34 Tkinter tests and both **786 / 786** complete discovery forms with one expected skip; localization remains 1,337/current, headless reports 3 updates, compileall/Git and the Dear PyGui/Tkinter/SalixTorrent visual smoke pass, and `pre_commit_check.bat` passes. Persistent expansion, drag/drop/reparenting, preview click-to-select, visual overlays, concrete property editors, file-dialog/template policy and final public API/schema naming remain later work.**
