...
**Current `dev` extraction — designer hierarchy projection/expansion:** Tranche 17 stable-ID hierarchy navigation is published at `f07bc5cc0bda02b81ffefd35b2da3a47e1168c7d`. Tranche 18 adds a backend-neutral hierarchy presentation model above that navigation seam: `DesignerHierarchyProjection` owns ephemeral expanded IDs and projects visible `DesignerHierarchyRow` records with depth, parent/child metadata and synchronized selected/focused flags. Root expansion is useful by default, explicit expand/collapse/toggle operations never dirty or enter undo history, and reveal expands only the ancestors needed to expose a stable-ID target. Expansion reconciles after accepted structural edits but is not persisted to designer project files or replayed by undo/redo. Preview-host hierarchy operations never rebuild the component tree; a real Tkinter proof exercises selection/reveal/collapse against the rendered blank-application hierarchy with unchanged preview generation. Real-Windows acceptance passes 12 / 12 hierarchy-projection tests, 127 / 127 combined designer/relocation tests, 64 / 64 GUI-component tests, 28 / 28 Tkinter tests and both **720 / 720** complete discovery forms with one expected skip; localization remains 1,337/current, headless/compileall/Git checks pass, the manual visual smoke was completed before automated validation, and `pre_commit_check.bat` passes. This remains optional designer tooling: ordinary code-first applications continue composing semantic `Component` trees directly and never require designer hierarchy/project modules. Concrete toolkit tree widgets, persistent expansion state, keyboard binding policy, multi-selection, pointer hit-testing, drag/drop/resize handles and final public API/package naming remain deferred.**
