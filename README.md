...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` continues with post-v0.5.1 designer-surface tranches: Tranche 1 is published at `376ae76409bbdc47bda0c06e49ba8e3bed920203`, Tranche 2 is published at `079eaf6e05b678aa2708d83c69bb9cdc37db0433`, and Tranche 3 is real-Windows accepted with its one-commit documentation fold prepared. The current accepted Windows baseline is **807 / 807** in both Tranche-3 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` should advance only by an explicit release decision.

## Highlights

**v0.5.1 milestone — backend-neutral designer/RAD semantic core:** the post-v0.5.0 extraction now reaches a complete shell-facing semantic chain: stable designer metadata and identity, validated property/structural editing with undo/redo, snapshot reconstruction, transactional preview ownership, clipboard/duplicate, selection/focus, project-file ownership, stable-ID hierarchy navigation and projection, property-inspector presentation, workspace coordination, and backend-neutral editor-shell commands. These layers remain optional tooling over the same semantic `Component` / runtime / backend contracts used by code-first applications; concrete Dear PyGui/Tkinter editor surfaces, file-dialog/template policy, pointer-driven editing and final package/API naming remain later work.
...
**Current `dev` extraction — post-v0.5.1 designer property-inspector/editor surface:** Tranche 2 is published at `079eaf6e05b678aa2708d83c69bb9cdc37db0433` (`Add designer hierarchy-panel surface`). Tranche 3 adds `DesignerInspectorPanel`, a renderer-neutral presentation coordinator over the accepted selected-node inspector projection, plus explicit Dear PyGui and Tkinter property-editor hosts. Toolkit controls never become property/document owners: edits, clears, explicit `None`, validation, undo/redo, dirty state and preview replacement continue through `DesignerWorkspace` / the existing edit-session and preview-host contracts. The designer-shell proof now presents hierarchy, reconstructed preview and inspector side-by-side while retaining Designer Commands; hierarchy selection retargets the inspector through a presentation-only notification. Real-Windows acceptance passes 18 / 18 inspector-panel tests, 206 / 206 designer/relocation tests, 64 / 64 GUI-component tests, 36 / 36 Tkinter tests and both **807 / 807** complete discovery forms with one expected skip. Localization remains 1,337/current, headless reports 3 updates, compileall/Git checks, Dear PyGui/Tkinter/SalixTorrent visual smoke and pre-commit pass. The one-commit acceptance fold is prepared under `Add designer property-inspector surface`. Multi-selection/mixed values, specialized callback/service/binding editors, file-dialog/template policy, pointer selection/overlays, drag/drop/resize, multi-document/autosave and final public API/schema naming remain later work.**
...
- Python 3.13 is the primary development/runtime version for v0.5.1.
- Dear PyGui 2.3.1 or newer in the 2.x series.
- aiohttp 3.11 or newer in the 3.x series.
- Tk support in the Python installation for the current native file/folder dialogs. Tkinter also provides the post-v0.5.0 compatibility backend for the common component/layout/scene/realtime-plot/live-data/command-menu surface, including local explicit placement, non-measuring overlays, responsive anchoring/constraints, keyed tabs and responsive split regions; SalixTorrent itself still uses Dear PyGui as its reference desktop backend.
...
The completed v0.5.0 extraction sequence advanced the real-Windows suite through the reusable component, documentation, geometry, package-relocation and responsive-coordination milestones. The released v0.5.0 baseline is **416 / 416 tests passing** with one expected non-Windows shell-behavior skip. Focused framework gates remain **63 component**, **14 responsive-layout**, **40 documentation/localization**, and **5 framework-packaging** tests.
...
v0.5.0 is a completed release checkpoint. It preserves the Phase 1-12 protocol, desktop, localization, persistence, packaging, tracked-test and seeding-goal milestones and adds the first physically separated reusable GUI/RAD foundation. The release is the stable `main` rollback boundary before broader engine/framework extraction.

Post-v0.5.0 development now uses the `dev` branch. The goal is broader than a widget toolkit: SalixTorrent is being used as a mature reference application to extract a modular application ecosystem consisting of a reusable **application engine/runtime**, an optional **RAD/application framework**, and concrete **presentation/platform adapters**.
...
The realtime Speed, runtime/network, second-backend blank-application, live-data/interaction, command-menu/mixed-layout, structural-region, responsive-anchor, designer-metadata, designer-editing and structural-hierarchy tranches are committed on `dev`. The snapshot-to-preview reconstruction tranche is published at `8d05ca8b059cef0ba386f324c215f2e80a9bfa83`; it reconstructs supported immutable designer snapshots into fresh backend-neutral component trees while remaining independent of Dear PyGui/Tkinter imports and deliberately avoiding application callbacks or live synchronization.
...
The complete architectural direction and extraction rules are documented in `SalixTorrent-Ecosystem-Architecture.md`.
