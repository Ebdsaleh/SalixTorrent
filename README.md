...
**Current `dev` extraction — structural regions and recursive mixed layout:** the command-menu/ordered-item/mixed-layout tranche is Windows-validated and pushed at `19ee1ba92826501e2061132e2a514a38862082a1` after passing 563 / 563 tests with one expected skip. The current prepared tranche adds renderer-neutral keyed tab containers, responsive weighted split panels, and non-measuring overlay placement. Real-Windows visual acceptance also hardened the backend bridge so split panes use deterministic parent-local sizing/placement and tab-change normalization tolerates Dear PyGui callbacks whose selected page must be read from the tab bar current value. SalixTorrent now proves those contracts in the real Download detail tabs/three-way General split and Help Contents/Glossary/two-pane documentation layout, while the product-neutral blank application proves tabs, splits, overlays, local explicit placement, command menus, live tables, state grids and realtime graphs through either GUI backend. Prepared complete discovery is **581 tests**; Dear PyGui remains SalixTorrent's reference desktop backend, while anchors/edge offsets, percentages, richer constraints and final ecosystem naming/API freeze remain deferred.
...
- Tk support in the Python installation for the current native file/folder dialogs. Tkinter also provides the post-v0.5.0 compatibility backend for the common component/layout/scene/realtime-plot/live-data/command-menu surface, including local explicit placement, non-measuring overlays, keyed tabs and responsive split regions; SalixTorrent itself still uses Dear PyGui as its reference desktop backend.
