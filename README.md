...
**Current `dev` extraction — post-v0.5.1 designer component-placement resolver:** Tranche 5 is published at `0b821dd1ec9e7b59ba6838ca8ede2dfdd4e1e323`. Tranche 6 adds `DesignerComponentPlacementSurface`, a renderer-neutral resolver for `DesignerComponentInsertRequest`: the pending request becomes an explicit parent / child-slot / index / relationship-metadata form, and only confirmation creates a new stable-ID node through the existing checked structural/history/preview transaction. Missing catalog metadata is carried into the self-describing snapshot as part of the same undoable command. Sparse visible creation defaults are deliberately provisional; component types that require a child template (for example split/placed/composite field structures) are reported as not yet directly creatable rather than guessed. Dear PyGui and Tkinter remain form adapters. Preparation passes 16 / 16 placement tests, 248 / 248 designer/relocation tests, 64 / 64 GUI-component tests, 40 / 40 Tkinter tests and **854 / 854** complete discovery; the full-commit real-Windows validator, visual smoke and `pre_commit_check.bat` remain authoritative before committing.**
