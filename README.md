...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` has published post-v0.5.1 designer-surface Tranches 1–7, most recently Tranche 7 at `8d1d241538e6beb2849994e4187bc940b1d17f20` (`Add designer structural commands and shortcuts`). The current accepted real-Windows baseline is **871 / 871** in both Tranche-7 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` advances only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 designer preview sizing/explicit resize controls:** Tranche 7 is published at `8d1d241538e6beb2849994e4187bc940b1d17f20`. Tranche 8 adds a preview-only component profile so generic controls have predictable human-scale visual defaults without mutating snapshots or the application renderer profile; explicit Width/Height/Fill values still override those defaults. The Dear PyGui Inspector now uses bounded editors with visible Apply/None/Default action rows, making the existing checked Width/Height edit path a usable explicit resize mechanism before pointer resize handles. Focused preparation passes 13 / 13 preview-sizing tests, 277 / 277 designer/relocation tests, 64 / 64 GUI-component tests, 42 / 42 Tkinter tests and **885 / 885** complete discovery in the preparation environment; the full-commit real-Windows validator, Tkinter gate, Dear PyGui/Tkinter visual smoke and `pre_commit_check.bat` remain authoritative before committing.**
Windows visual repair: the preview mount is child-window-backed to isolate child sizing, and Tkinter inspector refreshes preserve editor-column width.
