...
**Current `dev` extraction — post-v0.5.1 designer preview click-selection surface:** Tranche 3 is published at `6675ee00013e2b7f7b185843843f15d8f21b9427` (`Add designer property-inspector surface`). Tranche 4 adds `DesignerPreviewSelectionSurface`, a renderer-neutral presentation coordinator that exposes the current rendered preview as stable-ID click targets without becoming another selection or document owner. Dear PyGui performs adapter-owned hover hit-testing and draws a transient viewport outline; Tkinter binds native preview widgets and outlines the selected component mount. Preview clicks delegate selection/focus and hierarchy reveal through `DesignerWorkspace`, remain dirty/history/preview-generation neutral, and retarget the existing hierarchy and inspector surfaces. Whole-preview replacement still occurs only through the accepted checked edit/history transaction; the click surface explicitly refreshes and rebinds to the replacement native items afterward. Preparation passes 12 / 12 preview-selection tests, 218 / 218 designer/relocation tests, 64 / 64 GUI-component tests, 38 / 38 Tkinter tests and **821 / 821** complete discovery. This tranche ships as a full-commit bundle: the Windows validator, dual-backend visual smoke and `pre_commit_check.bat` must all pass before committing `Add designer preview click-selection surface`. Drag/drop/reparent/resize gestures, multi-selection/mixed values, specialized callback/service/binding editors, file-dialog/template/paste policy, multi-document/autosave and final public API/schema naming remain later work.**
