...
**Current `dev` extraction — designer project-file ownership/save-load:** Tranche 15 stable-ID selection/focus is published at `a0d621a8266cdf42b5b1355c417ec7d6d03bb297`. Tranche 16 adds the first deliberately small on-disk ownership boundary for optional designer tooling: `DesignerProjectDocument` wraps one immutable `DesignerSnapshot` in a strict provisional versioned JSON envelope, while `DesignerProjectFile` owns one `DesignerEditSession`, optional absolute path, dirty/save state and atomic save/save-as semantics. Duplicate JSON keys, non-finite values, unsupported versions, unknown envelope fields and invalid snapshots are rejected rather than normalized silently. Selection/focus, clipboard and undo/redo history remain ephemeral and are not persisted; callbacks, services, bindings, assets and toolkit objects remain outside this slice. The same reopened snapshot feeds the existing preview host and real Tkinter renderer, while ordinary code-first applications continue composing semantic `Component` trees directly and never need designer project files. Real-Windows acceptance passed **694 / 694** in both complete discovery forms with one expected skip, plus 12 / 12 project-file tests, 103 / 103 combined designer/relocation tests, 64 / 64 GUI-component tests and 26 / 26 Tkinter tests; manual visual smoke and pre-commit checks passed. The accepted implementation is `a86acae` (`Add designer project file ownership`), with publication pending the documentation closure and push. The file envelope, extension and broader project schema remain provisional; multi-document ownership, migrations, autosave/recovery, asset manifests, callback/service serialization and pointer-driven canvas gestures remain deferred.**
