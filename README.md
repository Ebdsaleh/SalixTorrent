...
**Current `dev` extraction — designer workspace coordination:** Tranche 19 property-inspector presentation is published at `35d0d4c0966aa9a3a6aafbe263edfbe8b383d7aa`. Tranche 20 adds a backend-neutral `DesignerWorkspace` coordinator and immutable `DesignerWorkspaceState` that compose project path/persisted/dirty state, undo/redo availability and labels, clipboard presence, stable selection/focus, hierarchy rows/expansion, inspector projection and preview generation into one shell-facing snapshot. Ownership does not move: `DesignerProjectFile` still owns file semantics, `DesignerEditSession` still owns immutable document/history/ephemeral editor state, and `DesignerPreviewHost` still owns checked transactional preview replacement. Save/Save-As never rebuilds the preview; selection/hierarchy/inspector reads remain history-neutral; document mutations still flow through the existing validated/transactional paths. Real-Windows acceptance passes 12 / 12 workspace tests, 151 / 151 combined designer/relocation tests, 64 / 64 GUI-component tests, 30 / 30 Tkinter tests and both **746 / 746** complete discovery forms with one expected skip. Localization remains 1,337/current, headless reports 3 updates, compileall/Git checks pass, the manual visual smoke was completed before automated validation, and `pre_commit_check.bat` passes. This remains optional RAD tooling: ordinary code-first applications continue constructing the same semantic `Component` trees directly. Concrete editor-shell widgets, pointer hit-testing, drag/drop/resize gestures, multi-document/autosave policy and final public API/package naming remain deferred.**
