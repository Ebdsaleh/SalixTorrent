...
> **Release status:** v0.5.0 is the current release checkpoint. It preserves the v0.4.0 protocol, persistence, localization, packaging, seeding-goal, and transfer-lifecycle foundation while adding the completed reusable GUI/RAD framework boundary: backend-neutral components, explicit value bindings/events/disposal, reusable documentation and geometry contracts, package-relocation readiness, and host-driven responsive coordination. The v0.5.0 Windows release gate is 416 / 416 tests with one expected non-Windows shell-behavior skip, followed by standalone GUI/CLI, portable ZIP, installer, and frozen-version verification before the annotated release tag is published.
...
The structural-composition tranche is pushed and Windows-validated at **362 / 362 tests passing**, with one expected non-Windows shell-behavior skip. The Create Torrent form/tooltip tranche is pushed and Windows-validated at **370 / 370**. The runtime-state tranche is pushed and Windows-validated at **374 / 374**. The transfer-dialog tranche is pushed and Windows-validated at **380 / 380**. The explicit Preferences binding tranche is pushed and Windows-validated at **385 / 385**. The renderer-neutral event/disposal tranche is pushed and Windows-validated at **392 / 392**. The first physical framework-boundary tranche is pushed and Windows-validated at **397 / 397**. The documentation/geometry extraction tranche is pushed and Windows-validated at **403 / 403**. The framework package-relocation tranche is pushed and Windows-validated at **408 / 408**. The backend-neutral responsive-coordination tranche is pushed and Windows-validated at **415 / 415**. Release preparation adds one packaging-version consistency regression, so the v0.5.0 release gate is **416 / 416**.
...
v0.5.0 is the current release checkpoint. It preserves the completed Phase 1-12 protocol, desktop, localization, persistence, packaging, tracked-test, and seeding-goal milestones and completes the reusable GUI/RAD foundation developed after v0.4.0. Linux/BSD/macOS native desktop behavior, full target-locale population/review, and final external framework naming/repository/API work remain later hardening or project-boundary tasks.

The v0.5.0 reusable GUI component foundation was developed incrementally after v0.4.0. The first tranche introduced backend-neutral primitives, semantic `AUTO`/`FILL` sizing, `ControlRow`/`ControlGrid`/`ControlColumn`, and reusable labeled-field/duration composites. The second tranche added `TextInput`, generic `LabeledField` accessory composition, and `NumericUnitField`, then migrated Preferences value controls onto the framework layer. The third tranche moved concrete component dimensions out of views into a renderer-selected `ComponentLayoutProfile`; it is committed/pushed and Windows-validated at 354/354 with visual parity confirmed. The fourth tranche extends ownership into reusable section/dialog structures and generic post-build attachments and is pushed/Windows-validated at 362/362. The fifth tranche adds a renderer-neutral `Tooltip` attachment, `ProgressBar`, profile-owned Create Torrent form metrics, and migrates the complete Create Torrent form onto the component/structural layer; it is pushed/Windows-validated at 370/370. The sixth tranche adds renderer-neutral runtime configuration/group state helpers, moves Create Torrent's value/enabled/visibility lifecycle off direct Dear PyGui calls, and is pushed/Windows-validated at 374/374. The seventh tranche applies the same proven contracts to the Open Magnet workflow and the ordinary remove/recheck/completion utility dialogs, including profile-owned dialog sizing, component-owned runtime transitions, progress overlays and backend-neutral dialog centering; it is pushed/Windows-validated at 380/380. The eighth tranche introduces an explicit synchronous `ValueBinding`/`BindingSet` contract, migrates Preferences collection/synchronization and status/connectivity updates off direct Dear PyGui, and is pushed/Windows-validated at 385/385. The ninth tranche adds renderer-neutral `ComponentEvent` callback metadata plus explicit renderer-owned component disposal, without introducing automatic observers or lifecycle management; it is pushed/Windows-validated at 392/392. The tenth tranche begins the first physical extraction: reusable property-cascade and component implementations live under the provisional internal `app.framework` boundary, SalixTorrent code imports that boundary directly, and the old `app.engine.components` path remains only as a compatibility facade. The concrete Dear PyGui renderer is now an engine/backend adapter selected explicitly by `GuiEngine` rather than a hidden framework default; that boundary is pushed/Windows-validated at 397/397. The eleventh tranche proves the same physical boundary with a second independent subsystem: pure geometry plus semantic documentation model/layout/typography contracts now live under `app.framework`, while the Dear PyGui documentation renderer and SalixTorrent runtime/media/typography services deliberately remain application/backend adapters. The twelfth tranche hardens that boundary for eventual package extraction: framework modules now use package-relative imports internally, an isolated relocation probe copies the whole tree to a differently named package and imports every module without SalixTorrent or Dear PyGui, and a standard-library-only audit keeps the reusable dependency direction explicit. This proves relocation ergonomics without selecting a final framework package name or external repository yet. The thirteenth tranche extracts the remaining toolkit-neutral responsive state: `LayoutHost` defines the injected backend operations and `LayoutCoordinator` owns keyed refresh/trigger semantics plus memoized geometry application, while `DearPyGuiLayoutHost` retains native callback signatures, handler registries, item-size reads, and configuration writes. Existing views still consume the SalixTorrent `ResponsiveLayout` singleton, now as a thin composition surface over those contracts. This boundary passed the Windows 415/415 gate and live GUI smoke, completing the reusable-GUI-foundation feature scope; v0.5.0 release preparation follows as a separate release-only change.

The binding contract is intentionally explicit: application code decides when to collect current control values and when to apply model/settings values back to controls. Per-binding transforms handle localized/canonical presentation conversion and type coercion, but there are no implicit subscriptions, background writes, or automatic two-way model mutation. Preferences is the first complete settings surface to use this contract, while native folder selection, networking services, responsive geometry and multi-control seeding-duration composition stay outside it. Component callbacks are explicit for the same reason: reusable controls emit semantic `ACTIVATE`/`CHANGE` events with optional application `event_data`, while Dear PyGui sender/app-data/user-data conventions remain inside the renderer bridge. Existing command-style actions opt into that contract through `action_callback(...)`.
...
Width, height and row spacing resolve independently. Named profile slots now provide the framework/application default layer, sparse component themes can override those defaults, and explicit instance layout remains the final override. Generic field composition follows `label -> primary control -> 0..n accessories`, which covers patterns such as `Network interface / VPN + ComboBox + Refresh`, `Active download slots + NumericStepper + 0 = Unlimited`, and `Download + NumericStepper + unit ComboBox` without creating one-off framework classes for every row shape. Structural composition now adds reusable `SectionPanel` and `Dialog` boundaries plus context-managed `ControlRow` / `ControlColumn` construction so existing views can migrate container ownership without an all-at-once rewrite. Renderer-adjacent tooltip creation is also behind the component renderer: generic `Tooltip` attachments carry explanatory text while SalixTorrent converts Help/Glossary terms through an application adapter. Create Torrent is the first complete ordinary form to use that attachment path together with framework-owned panels, rows, value controls and a reusable `ProgressBar`. Its runtime state now also flows through component `get_value`/`set_value`/`set_enabled`/`set_visible` helpers and `ComponentGroup`, leaving raw item IDs only for the existing responsive-geometry service. The same state contract is now proven on the Open Magnet workflow: input/progress/status/Add/Cancel transitions use component objects, `ProgressBar` owns overlay updates, and `Dialog` owns minimum-size/centering behavior through the renderer. Remove, removal-notice, force-recheck and completion-notice windows use the same reusable dialog/control/attachment boundaries. Component lifetime is also explicit: `Component.dispose()` delegates destruction to the active renderer, is idempotent, and clears the component's rendered root; stale backend handles are rejected before value/state operations and can be replaced by an explicit rebuild. The reusable implementation now resides under the provisional `app.framework` namespace and is audited against imports from SalixTorrent engine/view/logic/localization layers and Dear PyGui. Dear PyGui remains the current desktop backend, but its concrete adapter lives outside that reusable boundary; `GuiEngine` explicitly installs it and supplies the active SalixTorrent component profile at the application composition root. The same boundary now owns pure responsive geometry and semantic documentation model/layout/typography contracts. `ResponsiveLayout` remains the SalixTorrent singleton/composition facade over the backend-neutral `LayoutCoordinator` and concrete `DearPyGuiLayoutHost`, while `DocumentationRenderer` remains the concrete application renderer, so backend/resource/media ownership is not hidden behind a false generic abstraction. Intra-framework dependencies are package-relative and the complete framework tree is regression-tested after copy/rename into an isolated temporary package. Dear PyGui callback signatures, item-handler registries, item-size reads, existence checks, and configuration writes remain adapter-owned; toolkit-neutral callback coordination and memoized geometry policy live in the framework.
...
│   │       └── state.py
│   ├── engine/
│   │   ├── components/              # compatibility facade during extraction
│   │   ├── component_renderers/
│   │   │   └── dearpygui.py
│   │   ├── documentation/              # compatibility + concrete renderer
│   │   │   ├── layout.py               # compatibility facade
│   │   │   ├── model.py                # compatibility facade
│   │   │   ├── renderer.py             # Dear PyGui/application adapter
│   │   │   └── typography.py           # compatibility facade
