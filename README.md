...
> **Release status:** the reference release remains **v0.5.1**, tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`. Active `dev` has published post-v0.5.1 designer-surface Tranches 1–10, most recently Tranche 10 at `176aa7e52c4607094bc7f95c56de2cdbabfd216e` (`Add designer numeric scrubbing and reset controls`). The current accepted real-Windows baseline is **921 / 921** in both Tranche-10 complete discovery forms with one expected non-Windows shell-behavior skip. `v0.5.0` remains the previous stable tag at `d403c47f98f7e30d8adf879cd04e098dabc8767e`; `main` advances only by an explicit release decision.
...
**Current `dev` extraction — post-v0.5.1 layout autonomy + first hierarchy drag/reparent:** Tranche 10 is published at `176aa7e52c4607094bc7f95c56de2cdbabfd216e` after real-Windows acceptance of Reset all to defaults, Inspector numeric scrubbing and shared normal/Shift/Ctrl resize sensitivity. Tranche 11 makes linear-container cross-axis sizing explicit: `natural` is the default so one child's width/height does not silently stretch its siblings, while `stretch` remains an intentional parent policy. It also adds the first hierarchy drag/reparent gesture for unambiguous metadata-free container slots, resolving stable source/target IDs through the existing immutable structural transaction/history model; grids/tabs/splits/positioned relationships still use explicit Placement. Preparation targets **946 / 946** complete discovery with a 23-test Tranche-11 focused gate, 334 designer/relocation tests and 46 Tkinter tests; real-Windows Dear PyGui drag/drop and visual geometry remain authoritative before commit.**
