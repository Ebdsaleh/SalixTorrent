...
> **Release status:** v0.5.0 is released and tagged at `d403c47f98f7e30d8adf879cd04e098dabc8767e`. The real-Windows release gate passed 416 / 416 tests with one expected non-Windows shell-behavior skip, and the standalone GUI/CLI, portable ZIP and Inno Setup installer were built and smoke-tested with frozen version output reporting `SalixTorrent 0.5.0`. Post-v0.5.0 architecture work now develops on the `dev` branch while `main` remains the stable release line.
...
**Current `dev` extraction — designer property-inspector presentation:** Tranche 18 hierarchy projection/expansion is published at `9bd56a7e4a758529a54bbf45c3cc98bc061fa58c`. Tranche 19 adds a backend-neutral property-inspector presentation model above the already-proven `DesignerPropertyState` and stable-ID selection contracts. `DesignerPropertyInspector` projects the selected node into immutable `DesignerInspectorState` / `DesignerInspectorRow` records carrying type header metadata, ordered property rows, semantic editor hints, current value/set state, choice/bounds metadata and derived edit/clear affordances. Inspector projection and selection remain ephemeral and history-neutral; selected-property edits still route through the existing validated command/undo/redo path, and preview-host edits retain transactional rebuild semantics. Project save/load does not persist an inspector target, and a real Tkinter proof exercises selected-property replacement while preserving stable selection. Real-Windows acceptance passes 12 / 12 inspector tests, 139 / 139 combined designer/relocation tests, 64 / 64 GUI-component tests, 29 / 29 Tkinter tests and both **733 / 733** complete discovery forms with one expected skip; localization remains 1,337/current, headless/compileall/Git checks pass, the manual visual smoke was completed before automated validation, and `pre_commit_check.bat` passes. This remains optional designer tooling: ordinary code-first applications continue composing semantic `Component` trees directly and never require designer inspector/project modules. Concrete toolkit inspector widgets, multi-selection/mixed-value editing, property categories/search, pointer hit-testing, drag/drop/resize handles and final public API/package naming remain deferred.**
