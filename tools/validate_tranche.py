...
def validation_commands(python_executable: str | Path) -> tuple[ValidationCommand, ...]:
    """Return the ordered post-v0.5.1 Tranche-5 validation sequence."""
...
def run_validation(report_path: Path) -> int:
    ...
    with report_path.open("w", encoding="utf-8", newline="\n") as report:
        ...
        _write_line(report, "Designer shell + component palette + hierarchy + inspector + clickable preview Dear PyGui smoke:")
        _write_line(report, r"  python examples\ecosystem_designer_shell.py --ui-backend dearpygui")
        _write_line(report, "Designer shell + component palette + hierarchy + inspector + clickable preview Tkinter smoke:")
