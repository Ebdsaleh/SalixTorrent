...
- Added 18 focused inspector-panel tests, updated copied/renamed framework relocation coverage and added real Tkinter editor/selection-retarget regressions. Real-Windows acceptance passes 18 / 18 inspector-panel tests, 206 / 206 combined designer/relocation tests, 64 / 64 GUI-component tests, 36 / 36 Tkinter tests and both complete discovery forms at 807 / 807 with one expected skip. Localization remains 1,337/current, the headless blank app reports 3 updates, compileall/Git checks, the Dear PyGui/Tkinter/SalixTorrent visual smoke and `pre_commit_check.bat` pass. Implementation and acceptance documentation are ready to be committed together under `Add designer property-inspector surface`.
- This tranche does not add multi-selection/mixed-value editing, specialized callback/service/binding editors, pointer hit-testing/click-to-select, visual selection overlays, drag/drop/reparent/resize gestures, file-dialog/template policy, multi-document ownership, autosave/recovery, incremental native-widget mutation or final public API/project-schema naming. Code-first composition remains first-class and independent of optional designer tooling.
