...
- Added focused hierarchy-panel tests, copied/renamed framework relocation coverage and real Tkinter interaction regressions. Real-Windows acceptance passes 12 / 12 hierarchy-panel tests, 187 / 187 combined designer/relocation tests, 64 / 64 GUI-component tests, 34 / 34 Tkinter tests and both complete discovery forms at 786 / 786 with one expected skip. Localization extraction remains current (1,337 entries), the headless blank app reports 3 updates, compileall/Git checks and the Dear PyGui/Tkinter/SalixTorrent visual smoke pass, and `pre_commit_check.bat` passes. Implementation and acceptance documentation are folded into the same final commit under `Add designer hierarchy-panel surface`.
