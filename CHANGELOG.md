...
- Preparation validation advances complete discovery from 591 to **602 tests**. Display-less canonical Linux discovery passes 602 / 602 with 58 expected GUI/platform skips; canonical localization remains 1,337 strings. Real-Windows acceptance is still required before commit/push.
- This tranche intentionally stops before component reconstruction/factories, a final project/document schema, property mutation, drag/drop/reparenting, copy/paste, command-based edits or undo/redo. No version bump, release tag, merge to `main`, final ecosystem naming or public API freeze is introduced.
