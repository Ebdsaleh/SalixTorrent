...
- Added Dear PyGui and Tkinter resize hosts under `app/engine/designer_preview_resize_hosts/`. Dear PyGui draws a transient bottom-right handle and owns mouse down/move/release hit-testing; Tkinter overlays a native `ttk.Sizegrip`. During a drag the host may resize the live preview item for feedback, but only release can commit document state, and stale callbacks are revalidated against the current stable selected ID.
- Integrated the resize surface into `examples/ecosystem_designer_shell.py`. Selection changes, inspector edits, placement, structural commands and preview replacement all rebind the handle to the current stable-ID component. A successful gesture rebuilds Preview, preserves selection and retargets Inspector through the same workspace ownership already used by explicit Width/Height edits.
- Repaired the remaining narrow Tkinter Inspector layout defect observed during Tranche-8 Windows acceptance. Apply / None / Default actions now occupy a second row beneath each editor instead of a third horizontal column that could extend past the visible inspector canvas. The editor column keeps its minimum width and geometry stabilization across Default/None refreshes.
- Added 18 focused pointer-resize regressions plus two live Tkinter regressions covering one-step width/height history, stable selection across rebuild/undo, stale callback rejection, renderer-neutral boundaries, native Sizegrip drag commit, constrained Inspector action visibility, and Dear PyGui resize/selection arbitration. The designer/relocation gate is 295 tests, the live Tkinter gate 44 tests and complete discovery 905 tests.
- Real-Windows automated validation of the first Tranche-9 build was otherwise green (15 / 15 pointer-resize, 292 / 292 designer+relocation, 44 / 44 Tkinter and both complete discovery forms 902 / 902 with one expected skip), but visual acceptance exposed a Dear PyGui handle-boundary defect: the square straddled the selected control edge, so pressing the outer half could route selection to its parent/root and retarget the gesture. The repaired DPG hosts share a transient pointer arbiter, keep the complete handle inside the selected component, select ordinary preview targets on pointer-down rather than release/click, block selection when the resize handle owns the press, and freeze the original stable-ID/native item for the full drag.
- Hierarchy drag/reparent, richer creation templates, multi-selection/mixed values and more advanced resize affordances remain later direct-manipulation work; this tranche intentionally proves one selected-component bottom-right resize gesture first.

### Ecosystem Extraction — Designer Preview Sizing and Explicit Resize Controls (post-v0.5.1 Tranche 8)

- Added a designer-preview-only layout profile layered over the active component renderer profile. Generic controls now render at deterministic human-scale defaults in visual previews (for example Button 120x28, Combo box 180 wide, Text input 220 wide, Numeric stepper 160 wide and Progress bar 220x22) instead of relying on backend-native AUTO geometry that could let Dear PyGui controls consume the available preview region.
- Preview defaults are presentation policy only: they are not written into `DesignerSnapshot`, do not dirty the project, do not add history and do not alter the application renderer's installed profile. Application-specific profile keys remain inherited, while explicit designer `layout.width` / `layout.height` values — including intentional `fill` — retain normal precedence.
- Made the existing Width/Height inspector path a practical explicit resize control on Dear PyGui. Inspector editors now use bounded widths and put Apply / None / Default actions on a separate action row instead of combining fill-width fields and buttons in one constrained table cell. Pressing Enter or Apply still delegates through the same checked workspace edit/history/preview transaction.
- Added 13 focused preview-sizing regressions covering profile inheritance, renderer-profile isolation, explicit-size and `fill` precedence, opt-out behavior, checked resize/rebuild, restoring defaults, headless neutrality and the bounded Dear PyGui inspector source contract. The designer/relocation gate grows to 277 tests, the live Tkinter gate to 42 tests and complete discovery to 885 tests.
- Windows visual re-test exposed two presentation-only defects before publication. The designer example no longer mounts the reconstructed preview inside a fill-sized `ControlColumn`/Dear PyGui group (which could propagate fill geometry into child controls); it now uses a child-window-backed `SectionPanel`, keeping pane extent separate from component size. The Tkinter inspector also reasserts its canvas-window width after every refresh and reserves a minimum editor column so clearing Width/Height/Spacing cannot collapse empty editors to unusable widths.
- Real-Windows acceptance passed the 13 / 13 sizing gate, 277 / 277 designer+relocation, 64 / 64 GUI components, 42 / 42 Tkinter and both complete discovery forms at 885 / 885 with one expected skip. The corrected DPG preview sizing and Tkinter refresh behavior were accepted, and Tranche 8 is published at `44745b0df1c9e41394ed7ab27674815cfe6d1ea3` (`Add designer preview sizing and resize controls`).
- Pointer resize handles remain deliberately deferred inside Tranche 8 itself. Tranche 9 consumes the now-proven `layout.width` / `layout.height` contract rather than introducing a second sizing model.

### Ecosystem Extraction — Selected-Node Structural Commands and Shortcuts (post-v0.5.1 Tranche 7)

- Extended the backend-neutral designer shell with explicit `Move Up`, `Move Down`, and `Remove` commands for the current non-root selection. Availability is projected from the current stable-ID location and sibling boundary; dispatch delegates through `DesignerWorkspace` and the existing checked structural/history/preview transaction rather than mutating toolkit trees.
- Added `DesignerWorkspace.remove_selected()`, `move_selected_up()`, and `move_selected_down()` as narrow selected-node helpers over the already-proven remove/reorder primitives. Reordering preserves stable selection identity; removal uses the existing selection reconciliation fallback and remains undoable.
- Added framework-only `app/framework/designer_shell_shortcuts.py` plus Dear PyGui/Tkinter hosts. The default structural bindings are `Ctrl+D` Duplicate, `Ctrl+Delete` Remove, `Alt+Up` Move Up, and `Alt+Down` Move Down. Concrete hosts translate native keys only; enablement and command effects are revalidated through `DesignerShellCommands`.
- Expanded the designer-shell proof to install the same shortcut surface alongside Designer Commands. This gives a familiar way to verify insertion/reordering/removal before drag/drop is introduced, while palette placement, hierarchy, preview selection, inspector editing and undo/redo retain their existing owners.
- Corrected the designer-shell proof layout so Components, Placement and Hierarchy occupy explicit framework-owned vertical split panes. This prevents Dear PyGui flow/scroll behavior from letting the Components palette consume the full left editor region while preserving the same composition on Tkinter. The Dear PyGui placement host also renders Parent / Slot / Index / Metadata as explicit left-hand captions beside fixed-width editors. This avoids both native trailing-label clipping and a Windows Dear PyGui table-cell fill-width overrun that could still push captions past the bounded placement pane.
- Added 16 focused structural-command/shortcut tests, framework relocation coverage, and one live Tkinter shortcut-host regression. Real-Windows acceptance passed 16 / 16 focused tests, 264 / 264 designer+relocation, 64 / 64 GUI components, 41 / 41 Tkinter and both complete discovery forms at 871 / 871 with one expected skip; the corrected three-surface sidebar/Placement layout was visually accepted and the tranche is published at `8d1d241538e6beb2849994e4187bc940b1d17f20` (`Add designer structural commands and shortcuts`).
- Drag/drop/reparent gestures remain deliberately deferred until the command/placement semantics are proven independently; richer creation templates, resize handles, multi-selection/mixed values, callback/service/binding editors and final public API/schema naming also remain later work.

### Ecosystem Extraction — Explicit Designer Component Placement (post-v0.5.1 Tranche 6)

- Added framework-only `app/framework/designer_component_placement.py` to resolve `DesignerComponentInsertRequest` through explicit parent, child-slot, insertion-index and relationship-metadata choices instead of hidden toolkit placement policy.
- Placement now allocates a collision-safe stable node ID and commits through the existing checked structural/history/preview transaction. When a new component type is absent from the current self-describing snapshot, its catalog descriptor is added in the same undoable command.
- Added conservative sparse creation defaults for visible primitive controls and explicit rejection for types that require mandatory child templates before they can be reconstructed. Grid/tab/split/positioned relationships receive editable metadata suggestions, while structural and preview validation remain authoritative.
- Added Dear PyGui and Tkinter placement hosts under `app/engine/designer_component_placement_hosts/`; native controls are disposable form state and never own document/history/selection semantics. The designer-shell proof now stacks Components / Placement / Hierarchy beside Preview and Inspector.
- Added 16 focused placement tests, copied/renamed framework relocation coverage and one real Tkinter placement-host regression. Real-Windows acceptance passed 16 / 16 placement, 248 / 248 designer+relocation, 64 / 64 GUI components, 40 / 40 Tkinter and both complete discovery forms at 854 / 854 with one expected skip; visual smoke and pre-commit passed, and the tranche is published at `9b61c6f61c8c7eb451a52b12283873d8a96555c0` (`Add explicit designer component placement`).
- Drag/drop/reparent gestures, resize handles, richer creation templates, multi-selection/mixed values, callback/service/binding editors, file-dialog/template/paste policy, multi-document/autosave/recovery and final public API/schema naming remain later work.

### Ecosystem Extraction — Designer Component-Palette Request Surface (post-v0.5.1 Tranche 5)

- Added framework-only `app/framework/designer_component_palette.py` as a catalog-backed component/tool palette surface. It projects `DesignerCatalog` entries through an injected host and emits immutable `DesignerComponentInsertRequest` objects instead of constructing nodes or guessing insertion parent, child slot, relationship metadata, index or placement policy. The current stable selection is carried only as a `target_hint`.
- Added Dear PyGui and Tkinter palette hosts under `app/engine/designer_component_palette_hosts/`. Dear PyGui renders grouped tool buttons; Tkinter renders a grouped `ttk.Treeview` with explicit activation. Native items remain disposable presentation bindings and never own document/history/selection state.
- Expanded `examples/ecosystem_designer_shell.py` so the left editor region now contains Components above Hierarchy while Preview and Inspector retain their accepted ownership. Activating a tool reports an explicit placement request; no document mutation occurs until a later placement surface resolves it.
- Fixed the SalixTorrent product menu regression where `View -> Torrent Details -> General/Peers/Pieces/Files/Sources/Speed` could dispatch an empty tab key. The Dear PyGui detail menu now supplies the semantic tab name explicitly through `user_data`, preventing the callback's third argument from overwriting the loop default. A structural regression test locks that callback contract.
- Added 14 focused component-palette tests, copied/renamed framework relocation coverage and one real Tkinter palette-host regression. Real-Windows acceptance passed 14 / 14 palette, 6 / 6 structural/menu routing, 232 / 232 designer+relocation, 64 / 64 GUI components, 39 / 39 Tkinter and both complete discovery forms at 837 / 837 with one expected skip; visual smoke and pre-commit passed, and the tranche is published at `0b821dd1ec9e7b59ba6838ca8ede2dfdd4e1e323` (`Add designer component palette and fix detail menu`).
- Tranche 5 deliberately stopped before resolving palette insertion placement; Tranche 6 takes that next explicit seam. Drag/drop/reparent/resize, multi-selection/mixed values, callback/service/binding editors, file-dialog/template/paste policy, multi-document/autosave/recovery and final public API/schema naming remain later work.

### Ecosystem Extraction — Designer Preview Click-Selection Surface (post-v0.5.1 Tranche 4)

- Added framework-only `app/framework/designer_preview_selection.py` as a presentation coordinator over the current rendered designer preview. `DesignerPreviewSelectionSurface` derives stable-ID click targets from the existing `DesignerPreviewHost` mapping, delegates selection/focus and hierarchy reveal back through `DesignerWorkspace`, and never owns document, history, inspector or preview state.
- Added explicit Dear PyGui and Tkinter preview-selection hosts under `app/engine/designer_preview_selection_hosts/`. Dear PyGui performs adapter-owned native hover hit-testing and draws a transient viewport selection outline; Tkinter binds native preview widgets and outlines the selected component mount. Both hosts treat those bindings/highlights as disposable presentation state rather than persisted designer metadata.
- Preview clicks are revalidated against the current stable-ID snapshot before dispatch. Selection remains dirty/history/preview-generation neutral, while accepted property/history operations that rebuild the whole preview are followed by an explicit surface refresh so click targets rebind to the replacement native items without changing designer identity.
- Expanded `examples/ecosystem_designer_shell.py` so the three-pane Hierarchy / Preview / Inspector proof now supports selection from either the hierarchy tree or the rendered preview. A preview click reveals the same node in the hierarchy, retargets the inspector and shows the native selection outline while keeping Designer Commands unchanged.
- Added 12 focused preview-selection tests, updated copied/renamed framework relocation coverage and added two real Tkinter regressions for native click dispatch/highlighting and rebinding after preview replacement. Real-Windows acceptance passes 12 / 12 preview-selection tests, 218 / 218 combined designer/relocation tests, 64 / 64 GUI-component tests, 38 / 38 Tkinter tests and both complete discovery forms at 821 / 821 with one expected skip; localization/headless/compile/Git, Dear PyGui/Tkinter/SalixTorrent visual smoke and pre-commit all passed. Implementation and documentation are published together at `31d0d4207c12ffd226bddd29a2a81d26c92e98dd` (`Add designer preview click-selection surface`).
- This tranche does not add drag/drop/reparent gestures, resize handles, multi-selection/mixed-value editing, callback/service/binding editors, file-dialog/template/paste-placement policy, multi-document/autosave/recovery, incremental native-widget mutation or final public API/project-schema naming. Code-first composition remains first-class and independent of optional designer tooling.

### Ecosystem Extraction — Designer Property-Inspector/Editor Surface (post-v0.5.1 Tranche 3)

- Added framework-only `app/framework/designer_inspector_panel.py` as the first concrete selected-node property-inspector/editor coordinator over the accepted `DesignerInspectorState` / `DesignerInspectorRow` projection. `DesignerInspectorPanel` reads current rows from one existing `DesignerWorkspace`; edits and clears delegate back through that workspace so snapshot validation, undo/redo, dirty state and transactional preview replacement remain authoritative.
- Added explicit Dear PyGui and Tkinter inspector hosts under `app/engine/designer_inspector_panel_hosts/`. The hosts render all accepted semantic editor kinds, expose distinct `None` and inherited/default affordances where metadata permits them, and treat native controls as disposable presentation bindings rather than property state. Shared renderer-neutral editor codecs keep scalar, dimension, list and inset text representations deterministic while final validation still runs through the existing designer edit contract.
- Extended `DesignerHierarchyPanel` with an optional presentation-only change notification so direct hierarchy selection can retarget a sibling inspector surface without moving selection or coordination ownership into either toolkit. Stale inspector callbacks revalidate the selected stable node ID before dispatch, preventing an old native editor from mutating a newly selected component.
- Expanded `examples/ecosystem_designer_shell.py` into a three-pane hierarchy / reconstructed preview / inspector proof while retaining the accepted Designer Commands surface. Property edits rebuild only through the existing checked preview transaction; hierarchy selection remains preview-neutral and retargets the inspector projection on both desktop backends.
- Added 18 focused inspector-panel tests, updated copied/renamed framework relocation coverage and added real Tkinter editor/selection-retarget regressions. Real-Windows acceptance passes 18 / 18 inspector-panel tests, 206 / 206 combined designer/relocation tests, 64 / 64 GUI-component tests, 36 / 36 Tkinter tests and both complete discovery forms at 807 / 807 with one expected skip. Localization remains 1,337/current, the headless blank app reports 3 updates, compileall/Git checks, the Dear PyGui/Tkinter/SalixTorrent visual smoke and `pre_commit_check.bat` pass. Implementation and acceptance documentation are published together at `6675ee00013e2b7f7b185843843f15d8f21b9427` (`Add designer property-inspector surface`).
- This tranche does not add multi-selection/mixed-value editing, specialized callback/service/binding editors, drag/drop/reparent/resize gestures, file-dialog/template policy, multi-document ownership, autosave/recovery, incremental native-widget mutation or final public API/project-schema naming. Code-first composition remains first-class and independent of optional designer tooling.

### Ecosystem Extraction — Designer Hierarchy-Panel Surface (post-v0.5.1 Tranche 2)

- Added framework-only `app/framework/designer_hierarchy_panel.py` as the first concrete hierarchy presentation coordinator over the accepted `DesignerHierarchyRow` projection. `DesignerHierarchyPanel` reads visible rows from one existing `DesignerWorkspace`, delegates selection/focus and expansion back to that workspace, and never owns document, history, selection, hierarchy, inspector or preview state.
- Added explicit Dear PyGui and Tkinter hierarchy-panel hosts under `app/engine/designer_hierarchy_panel_hosts/`. Toolkit items are disposable presentation bindings: the semantic hierarchy remains stable-ID workspace state, and a refresh may rebuild native rows without introducing an incremental toolkit-mutation contract.
- The Tkinter host uses a native `ttk.Treeview`; native select/open/close callbacks are deferred across the Tk event boundary and guarded during programmatic repopulation so widget-generated events cannot recursively become semantic state changes. Dear PyGui renders the same visible hierarchy projection with explicit expand/collapse and selection controls.
- Expanded `examples/ecosystem_designer_shell.py` into a two-pane designer-shell proof with a live hierarchy surface beside the reconstructed preview while retaining the accepted Designer Commands surface. The same semantic workspace drives both Dear PyGui and Tkinter.
- Added focused hierarchy-panel tests, copied/renamed framework relocation coverage and real Tkinter interaction regressions. Real-Windows acceptance passes 12 / 12 hierarchy-panel tests, 187 / 187 combined designer/relocation tests, 64 / 64 GUI-component tests, 34 / 34 Tkinter tests and both complete discovery forms at 786 / 786 with one expected skip. Localization extraction remains current (1,337 entries), the headless blank app reports 3 updates, compileall/Git checks and the Dear PyGui/Tkinter/SalixTorrent visual smoke pass, and `pre_commit_check.bat` passes. Implementation and acceptance documentation are published together at `079eaf6e05b678aa2708d83c69bb9cdc37db0433` (`Add designer hierarchy-panel surface`).
- This tranche does not add persistent hierarchy expansion, drag/drop or reparent gestures, preview pointer hit-testing/click-to-select, visual selection overlays, concrete property-editor controls, file-dialog/template policy, multi-document ownership, autosave/recovery or final public API/schema naming. Code-first composition remains first-class and independent of designer surface tooling.
...
- New hierarchy projections expand the root by default when it has children, support explicit expand/collapse/toggle operations, and reveal hidden nodes by expanding the root-to-parent ancestor path already defined by `DesignerHierarchyReveal`. Expansion is normalized in snapshot preorder and leaves are never treated as expandable.
- Integrated one session-owned hierarchy projection into `DesignerEditSession`. Expansion/reveal operations do not mutate `DesignerSnapshot`, dirty project files, create undo/redo entries, clear redo, or change clipboard/selection ownership. Visible rows read the existing stable-ID selection/focus state so hierarchy presentation stays synchronized without merging those concepts.
- Expansion state reconciles only after accepted document transitions. Surviving expandable IDs persist across property edits, moves, reparenting and undo/redo; removed nodes or nodes that become leaves are discarded. Undo restores document history but deliberately does not replay old expansion state as project data.
- Extended `DesignerPreviewHost` with hierarchy row/state and expand/collapse/reveal delegation. A real Tkinter regression proves a rendered `Actions` control can be selected, revealed, hidden by collapsing its hierarchy parent and still resolve to the same rendered component without increasing preview generation.
- Extended copied/renamed framework probes and tranche validation for the hierarchy-projection boundary. Real-Windows acceptance passes 12 / 12 hierarchy-projection tests, 127 / 127 combined designer/relocation tests, 64 / 64 GUI-component tests and 28 / 28 Tkinter tests. Both complete discovery forms pass **720 / 720** with one expected skip; localization remains 1,337/current, the headless/compileall/Git checks pass, the manual Dear PyGui/Tkinter/SalixTorrent smoke was completed before automated validation, and `pre_commit_check.bat` passes.
- Tranche 17 hierarchy navigation plus its documentation closure are published together at `f07bc5cc0bda02b81ffefd35b2da3a47e1168c7d`; `HEAD` and `origin/dev` matched there with a clean tree before this tranche.
- This tranche deliberately stops before concrete Dear PyGui/Tkinter hierarchy tree widgets, persistent expanded/collapsed state, platform keyboard shortcut policy, multi-selection/range selection, pointer hit-testing, visual selection overlays, drag/drop/reparent gestures, resize handles, incremental toolkit mutation or final public package/API naming. Code-first Python applications remain independent of designer hierarchy tooling.

### Ecosystem Extraction — Designer Hierarchy Navigation

- Added standard-library-only `app/framework/designer_navigation.py` for stable-ID hierarchy traversal over immutable designer snapshots. `DesignerHierarchyNavigator` exposes parent, first/last child, previous/next sibling, and previous/next preorder navigation without retaining toolkit tree items or live component instances.
- Added `DesignerHierarchyReveal` so hierarchy surfaces can expand the root-to-parent ancestor path needed to reveal a node without persisting expansion state into project documents or edit history. Navigation order follows the snapshot's existing child order and never wraps implicitly at hierarchy boundaries.
- Integrated navigation with `DesignerEditSession` as ephemeral editor interaction state. Selection and focus can move independently through relative targets, or be coupled explicitly; navigation/reveal operations do not dirty the document, create undo entries, invalidate redo, or mutate the immutable snapshot.
- Extended `DesignerPreviewHost` with navigation/reveal delegation. Stable-ID navigation resolves the currently rendered selected component while leaving preview generation unchanged; a real Tkinter regression proves parent/child navigation over the blank application's rendered hierarchy without rebuilding it.
- Updated copied/renamed framework probes and tranche validation for the navigation boundary. Real-Windows Tranche-17 acceptance passed 12 / 12 navigation tests, 115 / 115 combined designer/relocation tests, 64 / 64 GUI-component tests and 27 / 27 Tkinter tests. Both complete discovery forms passed **707 / 707** with one expected skip; localization remained 1,337/current, headless/compileall/Git checks passed, the manual visual smoke was completed before automated validation, and `pre_commit_check.bat` passed. The accepted implementation commit is `34b2084` (`Add designer hierarchy navigation`); implementation and documentation closure were published together at `f07bc5cc0bda02b81ffefd35b2da3a47e1168c7d`.
- Tranche 16 project-file ownership plus its documentation closure are published together at `d95f6a1987aaff1bda9fb270c6219b36b8ee2080`; `HEAD` and `origin/dev` matched there with a clean tree before this tranche.
- This tranche deliberately stops before toolkit-owned tree widgets, persistent expansion state, hierarchy keyboard binding policy, multi-selection/range selection, visual selection overlays, pointer hit-testing, drag/drop/reparent gestures, resize handles, incremental toolkit mutation, or final public package/API naming. Code-first Python applications remain independent of designer navigation tooling.

### Ecosystem Extraction — Designer Project File Ownership / Save-Load

- Added standard-library-only `app/framework/designer_project.py` as a deliberately small optional persistence boundary around the already-proven immutable `DesignerSnapshot` model. `DesignerProjectDocument` adds only a provisional versioned project envelope (`kind`, `version`, `snapshot`) rather than freezing a complete RAD application schema.
- Added strict deterministic project parsing/serialization. Project JSON is sorted UTF-8, rejects duplicate object keys and non-finite numbers, rejects missing/unexpected envelope fields and unsupported project versions, and reuses the existing `DesignerSnapshot` validators for node/type/relationship structure instead of accepting partially valid documents.
- Added `DesignerProjectFile` ownership around one `DesignerEditSession`. New unsaved projects remain dirty until first persistence; opened projects begin clean; edits reuse the existing snapshot dirty baseline; Save As changes the owned path only after a successful write. No filename extension is mandated while project naming/schema remain provisional.
- Project writes use a temporary file in the destination directory followed by `os.replace(...)`. The session clean baseline and owned path are committed only after replacement succeeds, and failed writes clean up the temporary file without discarding unsaved edits or stealing ownership.
- Kept editor-only state outside persisted data. Selection/focus, clipboard contents and undo/redo history are not serialized; a reopened file receives a fresh edit session around the saved snapshot. Callbacks, services, bindings, assets and concrete toolkit objects remain outside this persistence slice.
- Added code-first and preview proofs: the blank application's captured component tree round-trips through a project file without mutating the live source tree; a reopened project feeds the existing `DesignerPreviewHost`; and a real Tkinter regression renders the reopened blank-application snapshot through the normal preview/backend contracts.
- Extended the copied/renamed framework probe and tranche validator for the project-file boundary. Real-Windows Tranche-16 acceptance passed 12 / 12 project tests, 103 / 103 combined designer/relocation tests, 64 / 64 GUI-component tests and 26 / 26 Tkinter tests. Both complete discovery forms passed **694 / 694** with one expected skip; localization remained 1,337/current, headless/compileall/Git checks passed, manual visual smoke passed and `pre_commit_check.bat` passed. The accepted implementation commit is `a86acae` (`Add designer project file ownership`); implementation and documentation closure were published together at `d95f6a1987aaff1bda9fb270c6219b36b8ee2080`.
- Tranche 15 selection/focus plus its documentation closure are now published together at `a0d621a8266cdf42b5b1355c417ec7d6d03bb297`; `HEAD` and `origin/dev` matched there with a clean tree before this tranche.
- This tranche deliberately stops before a final RAD project schema/extension, multiple documents/windows, asset/resource manifests, callback/service/binding serialization, migrations, autosave/recovery, recent-project UI, multi-selection, pointer hit-testing, drag/drop/resize handles or final public package/API naming. Code-first Python applications remain independent of designer project files.

### Ecosystem Extraction — Designer Selection / Focus State

- Added standard-library-only `app/framework/designer_selection.py` for optional designer interaction state. `DesignerSelectionModel` stores only stable designer node IDs for one selected node and one focused node; it never retains toolkit handles, preview component instances, application services or serialized project state.
- Kept selection and focus deliberately distinct. Designer surfaces can keep an object selected while focus moves independently, or couple them explicitly through select-and-focus helpers. Selection/focus changes do not dirty the document, create undo history, clear redo history or rebuild previews.
- Integrated selection reconciliation into `DesignerEditSession`. Property edits, moves and reparenting retain stable IDs; when a selected/focused node is removed, state falls back to the nearest surviving ancestor from the previous document. Undo/redo restores document snapshots without treating ephemeral selection state as historical project data.
- Extended `DesignerPreviewHost` with stable-ID selection/focus helpers plus `selected_component` / `focused_component` lookup against the currently owned preview. Whole-preview replacement therefore swaps component objects while the selected/focused designer IDs remain stable. External session edits may temporarily outrun a stale preview until explicit `sync()`, without storing stale component references.
- Added a real Tkinter proof that a selected `Actions` control survives transactional preview replacement by ID: the old rendered component is disposed, the replacement component becomes the selected/focused preview object, and the code-first source hierarchy remains independent of designer state.
- Updated framework relocation and validation probes so selection/focus remains package-relative, standard-library/framework-only and usable after copying/renaming the framework without SalixTorrent or a concrete GUI toolkit.
- Real-Windows Tranche-15 acceptance passed 12 / 12 selection/focus tests, 91 / 91 combined designer/relocation tests, 64 / 64 GUI-component tests and 25 / 25 Tkinter tests. Both complete discovery forms passed **681 / 681** with one expected skip, localization remained 1,337/current, headless/compileall/Git checks passed, manual Dear PyGui/Tkinter/SalixTorrent smoke passed and `pre_commit_check.bat` passed. The accepted implementation commit is `98f64e2` (`Add stable designer selection and focus state`); implementation and documentation closure were published together at `a0d621a8266cdf42b5b1355c417ec7d6d03bb297`.
- This tranche intentionally stops before multi-selection, range/group selection, visual selection overlays, keyboard traversal policy, pointer hit-testing, drag/drop/resize handles, project persistence/schema, callback/service serialization, incremental toolkit mutation or final ecosystem/package API freeze.
