...
- Pointer resize handles remain deliberately deferred. Tranche 8 makes default geometry trustworthy and explicit Width/Height resizing visible/usable first; later direct-manipulation handles must resolve into the same `layout.width` / `layout.height` edit contract rather than owning a second sizing model.
