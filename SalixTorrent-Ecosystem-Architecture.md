...
Tranche 8 added 13 focused sizing regressions and raised the designer/relocation, live Tkinter and complete-discovery expectations to 277, 42 and 885 respectively. Real-Windows acceptance passed those gates with one expected discovery skip and the tranche is published at `44745b0df1c9e41394ed7ab27674815cfe6d1ea3`. Pointer handles remained outside Tranche 8 itself; Tranche 9 consumes the accepted sizing contract so the handle remains gesture/presentation state and commits through this same width/height path.

### Selected-component pointer resize surface — post-v0.5.1 Tranche 9 full-commit gate

Direct resizing is now an adapter-driven gesture over the already accepted semantic sizing model. Framework-only `DesignerPreviewResizeSurface` derives one ephemeral target from the current stable selection and the current rendered preview component. It does not own pointer state, renderer items, selection, project data or history. On release it revalidates the stable ID and sends one composite Width/Height edit through the existing checked workspace/preview transaction, giving the whole drag exactly one undoable `Resize component` history entry.

Concrete Dear PyGui and Tkinter hosts own the mechanics. Dear PyGui draws a transient viewport handle at the selected component's bottom-right corner and uses native mouse down/move/release handlers; Tkinter overlays a `ttk.Sizegrip` and binds the corresponding pointer events. Hosts may mutate only the disposable current preview item while the pointer moves so feedback is immediate. No snapshot change occurs until release, and a successful checked edit replaces/rebinds the preview by stable designer ID.

The same tranche fixes the remaining Tkinter Inspector width failure observed after Tranche 8. The previous three-column `label | editor | actions` grid could place Apply/None/Default outside a constrained inspector canvas. Actions now occupy a second row beneath the editor, while the existing minimum editor width and asynchronous canvas-width stabilization remain in force. This is presentation geometry only; inspector edit/clear semantics are unchanged.
...
Real-Windows Tranche-10 acceptance passed 33 / 33 reset/scrub, 19 / 19 pointer-resize regressions, 311 / 311 designer/relocation, 64 / 64 GUI components, 44 / 44 Tkinter and both complete discovery forms at 921 / 921 with one expected skip. It is published at `176aa7e52c4607094bc7f95c56de2cdbabfd216e`. The remaining sibling-width coupling observed in Tkinter is not a shared child dimension; it is an implicit parent cross-axis stretch policy and is therefore corrected at the layout-contract layer in Tranche 11.


### Linear-container geometry autonomy and first hierarchy drag/reparent — post-v0.5.1 Tranche 11 full-commit gate

Linear flow ownership is now explicit on both axes. A `ControlColumn` owns vertical ordering and a `ControlRow` owns horizontal ordering, but neither silently owns every child's cross-axis size. New framework `CrossAxisMode` policy defaults to `natural`, which preserves resolved child geometry. `stretch` is the deliberate parent opt-in for filling the available cross axis; a child's explicit `fill` request continues to override natural sizing. This distinguishes “the parent became wide enough to contain a wide child” from “all children must become equally wide”.

The policy is stored in component/designer metadata, reconstructed by the existing preview bridge and translated by concrete renderers. Tkinter no longer applies unconditional `fill='x'` to every column child or the analogous row behavior. Dear PyGui uses the same semantic flag when it can size the native child; primitives without a generic native dimension remain natural rather than gaining toolkit-specific fake sizing. Older snapshots with no `cross_axis` property retain the new natural default without migration.

Hierarchy direct manipulation uses the opposite ownership rule: the toolkit reports intent but never decides structure. `plan_hierarchy_reparent(...)` receives a source stable ID and target stable ID, resolves the target's current catalog child-slot contract and returns a plan only when the target has one repeatable/single available metadata-free slot. The hierarchy presenter executes that plan through `DesignerWorkspace.reparent_node(...)`, which delegates to the already accepted immutable `ReparentDesignerNode` checked transaction. Stable ID, undo/redo, dirty state, preview replacement, selection/focus and reveal therefore remain existing owners.

The first gesture is intentionally “drop onto container to append”. It does not guess grid coordinates, tab-page keys, split-pane keys, positioned coordinates or anchored relationship data. Those complex relationships are rejected with a Placement-directed error. Dear PyGui uses native drag payload/drop targets carrying only the node ID; Tkinter uses a small motion threshold over the Treeview and forwards source/target IDs on release. Neither adapter imports structural edit commands.

Preparation passes 23 / 23 focused layout/drag tests, 334 / 334 designer+relocation, 64 / 64 GUI components, 46 / 46 Tkinter, 946 / 946 complete display-less discovery and 946 / 946 complete Xvfb discovery. Real-Windows acceptance then confirmed independent sibling geometry and the tranche was committed/pushed as `7bc5192ccbd486483967351e016a066d3e94d9ce` (`Add designer layout autonomy and hierarchy reparenting`) with `HEAD == origin/dev`.

### Ephemeral live Inspector scrub preview — post-v0.5.1 Tranche 12 full-commit gate

The numeric scrub interaction now has an explicit two-phase ownership model. Pointer motion produces **preview drafts**; pointer release produces the semantic edit. `DesignerPreviewHost.preview_edit(...)` applies a candidate command to the authoritative `DesignerEditSession.snapshot`, reconstructs/builds that candidate through the existing preview bridge, and swaps it into the preview slot without calling the edit session or touching undo/redo/dirty state. Because every motion sample starts from the authoritative snapshot, draft updates are absolute and cannot compound rounding or layout effects from previous frames.

`DesignerPreviewHost.has_preview_draft` is defined by a difference between the installed preview source snapshot and the current session snapshot. `cancel_preview_draft()` reconstructs the session snapshot and retires the candidate. Normal checked `execute(...)` remains authoritative on release; when the final command is a semantic no-op it explicitly cancels any outstanding draft so transient visual state cannot escape the gesture. A successful final command prepares one checked candidate, commits one history operation and installs that accepted preview exactly as before.

`DesignerInspectorPanel` exposes a narrow live-preview callback to concrete hosts. Dear PyGui and Tkinter scrub adapters still own only pointer motion/modifier collection and disposable editor text. During movement they translate through the shared `DesignerDragModifiers` contract and request one ephemeral property preview; on release they call the existing `on_set` callback once. The shell-level preview callback rebinds selection and resize surfaces after whole-tree candidate replacement, preserving stable-ID ownership despite native widget replacement.

This deliberately preserves the framework's existing whole-tree preview architecture rather than introducing backend-specific incremental mutation. Incremental renderer mutation may be optimized later if profiling proves necessary, but it is not allowed to become a second semantic state owner. Preparation passes 10 / 10 focused Tranche-12 regressions, 344 / 344 designer+relocation, 64 / 64 GUI components, 46 / 46 Tkinter under Xvfb and complete discovery at 956 / 956.
