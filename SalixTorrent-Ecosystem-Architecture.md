...
### Stable-ID hierarchy navigation — Tranche 17 prepared
...
The real blank-application proof remains code-first. Navigation operates on the captured designer document/preview only, while a Tkinter regression proves rendered parent/child traversal can change stable selected/focused IDs and resolve the corresponding rendered component without increasing preview generation. The relocated framework probe uses the same navigator after package rename. Prepared gates are 12 navigation tests, 115 combined designer/relocation tests, 64 GUI-component tests, 27 Tkinter tests and 707 complete-discovery tests.
