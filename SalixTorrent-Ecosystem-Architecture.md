...
**Status:** v0.5.1 released; post-v0.5.1 concrete designer surfaces in progress on `dev`
...
The designer-shell proof now composes a live hierarchy panel beside the reconstructed preview while retaining the Tranche-1 command surface. A row selection updates the existing stable selection/focus owner without rebuilding the preview; expansion and reveal remain ephemeral; document-changing shell commands still use the previously accepted checked preview transaction. Relocation coverage exercises the framework presenter after package copy/rename, and real Tkinter tests drive native hierarchy events through the same semantic boundary. Real-Windows acceptance passes 12 / 12 hierarchy-panel tests, 187 / 187 combined designer/relocation tests, 64 / 64 GUI-component tests, 34 / 34 Tkinter tests and both complete discovery forms at **786 / 786** with one expected skip; localization/headless/compile/Git/manual Dear PyGui/Tkinter/SalixTorrent visual/pre-commit gates also pass. Implementation and acceptance documentation are folded into the same final commit under `Add designer hierarchy-panel surface`.
