...
The real blank-application proof remains code-first. Navigation operates on the captured designer document/preview only, while a Tkinter regression proves rendered parent/child traversal can change stable selected/focused IDs and resolve the corresponding rendered component without increasing preview generation. The relocated framework probe uses the same navigator after package rename. Real-Windows acceptance passes 12 / 12 navigation tests, 115 / 115 combined designer/relocation tests, 64 / 64 GUI-component tests and 27 / 27 Tkinter tests; both complete discovery forms pass **707 / 707** with one expected skip. Localization remains 1,337/current, headless/compileall/Git checks pass, the manual visual smoke was completed before automated validation, and `pre_commit_check.bat` passes. The accepted implementation commit is `34b2084`; publication is pending this documentation closure and push.
