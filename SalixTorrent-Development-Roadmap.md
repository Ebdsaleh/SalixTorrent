...
**Roadmap status:** v0.5.1 is released/tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`; post-v0.5.1 Tranches 1 through 3 are published and Tranche 4 preview click-selection uses the full-commit Windows-acceptance workflow
**Current implementation checkpoint:** post-v0.5.1 Tranche 4 designer preview click-selection is the current implementation/docs boundary over published Tranche 3 at `6675ee00013e2b7f7b185843843f15d8f21b9427`; publication is valid only after the real-Windows validator, visual smoke and pre-commit gate pass
**Current real Windows regression baseline:** 807 / 807 in both accepted post-v0.5.1 Tranche-3 discovery forms with one expected non-Windows shell-behavior skip; designer property-inspector/editor surface 18 / 18, designer/relocation 206 / 206, GUI components 64 / 64 and Tkinter 36 / 36; localization remains 1,337/current, headless reports 3 updates, Dear PyGui/Tkinter/SalixTorrent visual smoke and pre-commit passed.
...
**Post-v0.5.1 Tranche 4 full-commit gate:** preview click-selection 12 / 12, designer/relocation 218 / 218, GUI components 64 / 64, Tkinter 38 / 38 and both complete discovery forms 821 / 821; localization/headless/compileall/Git must remain clean, both designer backends plus the blank-app/product visual smoke must pass, and `pre_commit_check.bat` must pass before `Add designer preview click-selection surface` is committed.
...
### A28. Clickable preview selection and visual reveal — post-v0.5.1 Tranche 4 full-commit acceptance gate
...
Preparation passes 12 / 12 preview-selection tests, 218 / 218 combined designer/relocation tests, 64 / 64 GUI-component tests, 38 / 38 live Tkinter tests under Xvfb and complete discovery at **821 / 821**. The supplied full-commit bundle includes its documentation boundary up front: real-Windows `validate_tranche.bat`, both designer-shell backends, both blank-app backends, `python main.py` and `pre_commit_check.bat` must all pass before the single commit `Add designer preview click-selection surface` is created. If any automated or visual gate fails, the tranche must remain uncommitted for repair.
