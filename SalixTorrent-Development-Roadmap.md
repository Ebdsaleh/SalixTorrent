...
**Accepted post-v0.5.1 Tranche 1 baseline:** designer command-menu surface 12 / 12, designer/relocation 175 / 175, GUI components 64 / 64, Tkinter 32 / 32 and both complete discovery forms 772 / 772 with one expected skip; localization current, headless/compileall/Git checks, manual visual smoke and pre-commit passed; implementation and acceptance documentation are published together at `376ae76409bbdc47bda0c06e49ba8e3bed920203` under `Add designer command-menu surface`.
**Accepted post-v0.5.1 Tranche 2 baseline:** designer hierarchy-panel surface 12 / 12, designer/relocation 187 / 187, GUI components 64 / 64, Tkinter 34 / 34 and both complete discovery forms 786 / 786 with one expected skip; localization current, headless/compileall/Git checks, Dear PyGui/Tkinter/SalixTorrent visual smoke and pre-commit passed; implementation and acceptance documentation are published together at `079eaf6e05b678aa2708d83c69bb9cdc37db0433` under `Add designer hierarchy-panel surface`.
**Accepted post-v0.5.1 Tranche 3 baseline:** designer property-inspector/editor surface 18 / 18, designer/relocation 206 / 206, GUI components 64 / 64, Tkinter 36 / 36 and both complete discovery forms 807 / 807 with one expected skip; localization current, headless/compileall/Git checks, Dear PyGui/Tkinter/SalixTorrent visual smoke and pre-commit passed; implementation and acceptance documentation are published together at `6675ee00013e2b7f7b185843843f15d8f21b9427` under `Add designer property-inspector surface`.
...
### A23. WYSIWYG designer prerequisites — editor-shell commands Windows-accepted

Tranche 21 adds `app/framework/designer_shell.py` as the semantic command layer over the accepted `DesignerWorkspace`. `DesignerShellCommandState` projects one immutable File/Edit/Navigate command tree using the existing generic `CommandSpec` / `CommandSet` contracts. Availability and dynamic Undo/Redo labels are recalculated from current workspace/session state; stable hierarchy navigation uses the existing snapshot navigator rather than toolkit focus order.

`DesignerShellCommands` does not become a new owner. Direct Save delegates to `DesignerProjectFile`; Undo/Redo, Duplicate and hierarchy-changing document actions continue through the workspace/preview transaction; Copy and Reveal remain ephemeral. New/Open/Save-As and Paste return explicit `DesignerShellCommandRequest` values because project templates, file dialogs and paste placement belong to a future shell and are not yet framework policy. Navigation commands move stable selection/focus together and reveal the result without dirtying the project or rebuilding the preview.

The copied/renamed framework probe exercises the same command model without SalixTorrent or Dear PyGui, and a real Tkinter regression proves Copy remains preview-neutral while Duplicate and Undo reuse the established checked preview-replacement path. Real-Windows acceptance passes 12 / 12 shell-command tests, 163 / 163 combined designer/relocation tests, 64 / 64 GUI-component tests and 31 / 31 Tkinter tests; both complete discovery forms pass **759 / 759** with one expected skip. Localization remains 1,337/current, headless/compileall/Git checks pass, the manual visual smoke passed, and `pre_commit_check.bat` passes. This tranche deliberately stops before concrete menu/toolbar/palette widgets, file dialogs/templates, paste-placement UI, pointer-driven designer interaction or final public naming. Once published, the planned next step is a dedicated **v0.5.1 release closure/tag checkpoint**, not another backend-neutral designer prerequisite tranche.

### A24. Naming, public API and package/repository split
...
This tranche deliberately stops before persistent menu bars/toolbars, hierarchy tree widgets, concrete property editors, file-dialog/template policy, paste-placement UI, pointer hit-testing/click-to-select, visual selection overlays, drag/drop/reparent gestures, resize handles, multi-document ownership, autosave/recovery, mixed-value multi-selection or final project/package/API naming. Code-first applications remain independent of all designer surface tooling. Tranche 1 is published at `376ae76409bbdc47bda0c06e49ba8e3bed920203`.

### A26. Concrete designer hierarchy-panel surface — post-v0.5.1 Tranche 2 Windows-accepted

`app/framework/designer_hierarchy_panel.py` binds the already-accepted `DesignerHierarchyRow` projection to an injected `DesignerHierarchyPanelHost`. The presenter reads current visible rows from one `DesignerWorkspace`, delegates row selection/focus and expansion back through that workspace, and refreshes from semantic state after every accepted interaction. It owns no project document, history, selection, expanded-ID set, inspector or preview state. Stale callbacks are revalidated against the current visible-row projection instead of trusting old native widget state.

Concrete hosts live in `app/engine/designer_hierarchy_panel_hosts/`. Dear PyGui renders explicit indented expand/collapse and selectable row controls; Tkinter uses a native `ttk.Treeview`. Native Tk selection/open/close notifications are deferred across the event boundary and suppressed during programmatic repopulation so rebuilding presentation rows cannot recurse into semantic selection or expansion changes. Both hosts may replace their toolkit items wholesale during refresh; this tranche deliberately does not define an incremental toolkit-mutation API.

`examples/ecosystem_designer_shell.py` now gives the optional editor proof a two-pane composition: a live hierarchy surface on the left and the reconstructed semantic preview on the right, while the accepted Designer Commands surface remains available. Selection, expansion and reveal stay preview-neutral; document-changing commands continue through the existing workspace/preview checked transaction. The copied/renamed framework probe exercises the presenter without SalixTorrent or toolkit imports. Real-Windows acceptance passes 12 / 12 hierarchy-panel tests, 187 / 187 combined designer/relocation tests, 64 / 64 GUI-component tests and 34 / 34 Tkinter tests. Both complete discovery forms pass **786 / 786** with one expected skip; localization remains 1,337/current, headless reports 3 updates, compileall/Git checks pass, the Dear PyGui/Tkinter/SalixTorrent visual smoke passed, and `pre_commit_check.bat` passes.

This tranche deliberately stops before persisted expansion, drag/drop/reparenting, preview pointer hit-testing/click-to-select, visual selection overlays, concrete property-editor controls, file-dialog/new-project-template policy, paste-placement UI, multi-document ownership, autosave/recovery, incremental native-tree mutation or final project/package/API naming. Code-first applications continue to use the same semantic `Component` / runtime / backend contracts without any designer dependency.

### A27. Concrete designer property-inspector/editor surface — post-v0.5.1 Tranche 3 Windows-accepted / published

Tranche 3 adds `app/framework/designer_inspector_panel.py` as the first concrete editor surface over the already-accepted selected-node `DesignerInspectorState` / `DesignerInspectorRow` projection. `DesignerInspectorPanel` consumes one `DesignerWorkspace` plus an injected `DesignerInspectorPanelHost`, re-reads current property metadata on every refresh and delegates all edits/clears back to the workspace. It therefore owns no second property values, validation rules, history stack, dirty bit, selection ID or preview generation. Stale native callbacks carry the stable node ID they were built for and are rejected if selection has moved before dispatch.

Concrete Dear PyGui and Tkinter hosts live under `app/engine/designer_inspector_panel_hosts/`. They render the accepted semantic editor kinds (text, toggle, integer, number, choice, string/number lists, dimensions and insets), preserve distinct explicit-`None` and inherited/default affordances where metadata allows them, and may rebuild their native controls from the full inspector projection. Shared presentation codecs provide deterministic text forms for dimensions and JSON-backed collection/inset editing, but the existing designer edit/session validation remains authoritative. This tranche does not introduce toolkit-owned validation or a second serialization format.

The designer-shell proof becomes a three-pane hierarchy / preview / inspector composition while retaining the accepted Designer Commands surface. `DesignerHierarchyPanel` gains only an optional presentation notification so direct hierarchy selection can refresh a sibling inspector; selection itself remains workspace-owned and preview-neutral. Inspector edits continue through checked preview replacement and immediately participate in existing undo/redo and dirty-state semantics. Preparation passed 18 / 18 inspector-panel tests, 206 / 206 combined designer/relocation tests, 64 / 64 GUI-component tests, 36 / 36 live Tkinter tests under Xvfb and both complete discovery forms at **807 / 807**. Real-Windows acceptance then passed the same focused gates and both discovery forms with one expected skip; localization remained 1,337/current, headless/compileall/Git checks, Dear PyGui/Tkinter/SalixTorrent visual smoke and `pre_commit_check.bat` passed. The implementation and acceptance documentation are published together at `6675ee00013e2b7f7b185843843f15d8f21b9427` (`Add designer property-inspector surface`).

Tranche 3 deliberately stops before multi-selection/mixed-value editing, specialized callback/service/binding editors, drag/drop/reparent/resize gestures, file-dialog/new-project-template and paste-placement policy, multi-document/autosave/recovery, incremental native-widget mutation and final project/package/API naming. The permanent dual-entry architecture remains unchanged: code-first Python composition and optional designer/RAD authoring converge on the same semantic component/runtime/backend contracts.

### A28. Clickable preview selection and visual reveal — post-v0.5.1 Tranche 4 Windows-accepted / published

Tranche 4 adds `app/framework/designer_preview_selection.py` as a narrow presentation bridge over the already-rendered `DesignerPreviewHost` tree. `DesignerPreviewSelectionSurface` derives one ephemeral target per current stable designer node ID and delegates every click back through `DesignerWorkspace.select_and_focus_node(...)` plus the existing hierarchy reveal path. It does not persist hit regions, own another selected ID, alter document history or trigger a preview rebuild merely because the user selected something.

Concrete hosts live under `app/engine/designer_preview_selection_hosts/`. Dear PyGui keeps hit-testing entirely in the adapter by checking currently hovered rendered preview items and choosing the deepest semantic target, then draws a transient viewport rectangle around the selected item. Tkinter binds `<Button-1>` to current preview widgets/mounts and uses the renderer-created mount frame for a transient selected outline. Both hosts discard/rebuild those native bindings after whole-preview replacement, while stable designer IDs remain authoritative in the workspace.

The designer-shell proof keeps the three-pane Hierarchy / Preview / Inspector layout but now allows selection from either the hierarchy or the rendered preview. Clicking a preview control selects/focuses the same stable ID, reveals it in the hierarchy and retargets the inspector; hierarchy selection refreshes the preview outline. Property edits, duplicate/undo and other accepted document-changing operations still use the existing checked whole-preview transaction and explicitly refresh the click surface afterward so stale native items are never treated as semantic identity.

Real-Windows acceptance passes 12 / 12 preview-selection tests, 218 / 218 combined designer/relocation tests, 64 / 64 GUI-component tests, 38 / 38 Tkinter tests and both complete discovery forms at **821 / 821** with one expected skip. Localization/headless/compile/Git, both designer-shell backends, both blank-app backends, `python main.py` and pre-commit all passed. The full-commit boundary is published at `31d0d4207c12ffd226bddd29a2a81d26c92e98dd` (`Add designer preview click-selection surface`).

Tranche 4 deliberately stops before drag/drop/reparent gestures, resize handles, multi-selection/mixed-value editing, specialized callback/service/binding editors, file-dialog/new-project-template/paste-placement policy, multi-document/autosave/recovery, incremental native-widget mutation and final project/package/API naming. The preview selection outline is transient adapter presentation only and is not part of the saved designer document.

### A29. Component palette and explicit insertion-placement requests — post-v0.5.1 Tranche 5 Windows-accepted / published

Tranche 5 adds `app/framework/designer_component_palette.py` as a catalog-backed toolbox surface. `DesignerComponentPalette` projects `DesignerCatalog` entries and the current stable selection through a backend-neutral state model, but activating an entry produces an immutable `DesignerComponentInsertRequest` rather than constructing a node. The selected node is only a `target_hint`; parent, slot, index and relationship metadata remain deliberately unresolved so the framework does not guess placement policy.

Concrete Dear PyGui/Tkinter hosts live under `app/engine/designer_component_palette_hosts/`. The designer-shell proof stacks Components above Hierarchy in the left editor region, leaving Preview and Inspector ownership unchanged. Palette activation is document/history/preview neutral and reports the pending placement request in the shell status.

The same tranche repairs a user-reproduced SalixTorrent product bug in `View -> Torrent Details`: Dear PyGui supplies the selected semantic tab key as the menu item's explicit `user_data`, so the callback's third parameter can no longer replace the intended loop value with an empty value. A structural regression covers this contract.

Real-Windows acceptance passes 14 / 14 palette tests, 6 / 6 structural/menu-routing tests, 232 / 232 designer/relocation tests, 64 / 64 GUI-component tests, 39 / 39 Tkinter tests and both complete discovery forms at **837 / 837** with one expected skip. Visual smoke and pre-commit passed, and the implementation/docs are published at `0b821dd1ec9e7b59ba6838ca8ede2dfdd4e1e323` (`Add designer component palette and fix detail menu`). Tranche 5 intentionally stops before resolving a palette request into a concrete insertion.

### A30. Explicit component placement resolver — post-v0.5.1 Tranche 6 full-commit gate

Tranche 6 resolves the request seam without jumping to drag/drop. `app/framework/designer_component_placement.py` projects candidate parent nodes and compatible child slots from the current self-describing snapshot, carries an explicit insertion index plus JSON relationship metadata, and allocates a collision-safe stable node ID. The selected palette target remains only a hint: a selected leaf can suggest its current parent, while a selected container can be offered directly; the user can change parent and slot before committing.

Confirmation executes one checked structural command through the existing workspace/preview transaction. If the chosen component type is not already represented in the document, its catalog descriptor is added to the immutable snapshot in the same undoable history step, preserving self-description. A small set of visible sparse creation defaults makes newly inserted primitive controls identifiable; types that cannot be reconstructed without required child templates are explicitly rejected rather than fabricating nested structures. Relationship metadata suggestions cover first grid cells, tab-page keys, split-pane keys and fixed placement, but structural and preview validation remain authoritative.

Concrete Dear PyGui and Tkinter hosts under `app/engine/designer_component_placement_hosts/` own only disposable form controls. The designer-shell proof now stacks Components / Placement / Hierarchy beside Preview and Inspector. Preparation passes 16 / 16 placement tests, 248 / 248 designer/relocation tests, 64 / 64 GUI-component tests, 40 / 40 live Tkinter tests and complete discovery at **854 / 854**. Real-Windows validation and manual insertion smoke remain authoritative before commit.

Drag/drop, resize handles, richer creation templates, multi-selection/mixed values, callback/service/binding editors, file-dialog/template/paste policy, multi-document/autosave/recovery and final package/API/schema naming remain later work.

### A31. Selected-node structural commands and keyboard shortcuts — post-v0.5.1 Tranche 7 full-commit gate

Tranche 7 deliberately stays one step short of drag/drop. The accepted structural primitives already know how to remove and reorder immutable designer nodes, so `DesignerWorkspace` now exposes narrow selected-node helpers and `DesignerShellCommands` projects `Move Up`, `Move Down`, and `Remove` with availability derived from the current parent/sibling location. Every document-changing dispatch still passes through the checked preview transaction, history, stable-ID selection reconciliation and hierarchy reveal contracts.

`app/framework/designer_shell_shortcuts.py` adds a renderer-neutral gesture-to-command bridge rather than embedding editing semantics in toolkit key handlers. The default structural bindings are `Ctrl+D` Duplicate, `Ctrl+Delete` Remove, `Alt+Up` Move Up and `Alt+Down` Move Down. Dear PyGui and Tkinter hosts only translate native key events into those normalized gestures; disabled commands are inert and all effects are revalidated against live `DesignerShellCommands` state.

This tranche is intentionally useful as a verification bridge for Tranche 6. A newly inserted component can be reordered or removed through familiar menu/keyboard interactions, with Undo/Redo proving the same structural transaction underneath, even before pointer drag/drop exists. The designer-shell example uses a vertical `SplitPanel` for Components / Placement / Hierarchy so Dear PyGui cannot let one flow child consume the entire left editor region; Tkinter uses the same structural composition. Dear PyGui Placement captions are also rendered as explicit left-hand row labels beside conservative fixed-width controls rather than native trailing labels or table-cell fill-width editors, keeping Parent / Slot / Index / Metadata readable inside the negotiated sidebar width. Preparation passes 16 / 16 focused structural-shortcut tests, 264 / 264 designer/relocation tests, 64 / 64 GUI-component tests and complete discovery at **871 / 871**; the real-Windows 41 / 41 Tkinter gate and dual-backend visual smoke remain authoritative before commit.

A later drag/reparent gesture should resolve into these same structural commands and explicit parent/slot/metadata semantics rather than becoming a second source of document policy. Richer creation templates may also proceed independently where sparse component creation is insufficient.

### A32. Designer preview sizing and explicit resize controls — post-v0.5.1 Tranche 8 full-commit gate

Tranche 8 first fixes the visual sizing foundation exposed by the real Dear PyGui designer proof. Backend-native AUTO sizing was not sufficiently deterministic for the editor preview: an un-sized Button could occupy the available preview region even though the same semantic snapshot appeared at a conventional control size on Tkinter. `DesignerPreviewHost` now renders through a preview-only renderer view whose `ComponentLayoutProfile` layers human-scale generic control defaults over the active renderer profile. The base application renderer is never mutated, custom application profile keys still inherit normally, and explicit snapshot width/height values retain the normal profile -> theme -> instance precedence. The preview defaults are not serialized, dirty, or undoable document data.

The existing Inspector Width/Height properties remain the authoritative explicit sizing contract. Dear PyGui editor rows are adjusted so bounded editors and their Apply / None / Default actions remain visible in a constrained Inspector pane; entering Width/Height and applying still executes the same checked property-edit transaction and rebuilds Preview, while Default removes the explicit property and returns to the preview profile. Pointer resize handles are intentionally not a second semantic system and are deferred until this explicit path is visually proven.

Tranche 8 added 13 focused preview-sizing tests and raised the combined designer/relocation gate to 277 / 277, the live Tkinter gate to 42 / 42 and complete discovery to **885 / 885**. Real-Windows acceptance passed those gates with one expected complete-discovery skip, verified conventional Dear PyGui control sizing plus the corrected preview mount/Tkinter editor refresh behavior, and published the tranche at `44745b0df1c9e41394ed7ab27674815cfe6d1ea3` (`Add designer preview sizing and resize controls`).

Pointer handles remained outside Tranche 8 itself. Tranche 9 now consumes this accepted sizing contract so direct manipulation measures the selected rendered item only as gesture input and commits resulting semantic width/height through the same inspector/workspace transaction.

### A33. Selected-component pointer resize surface — post-v0.5.1 Tranche 9 full-commit gate

Tranche 9 turns the explicit Tranche-8 sizing contract into the first genuine direct-manipulation gesture. `DesignerPreviewResizeSurface` is backend-neutral and only projects the currently selected stable-ID preview component, its resolved rendered size and whether `layout.width` / `layout.height` are editable. Concrete hosts own the mouse pointer, transient bottom-right handle and live drag feedback. A release revalidates the selected stable ID and commits the resulting pixel dimensions once through `DesignerWorkspace.execute(CompositeDesignerEdit(...))`, so width and height become one undoable `Resize component` history step rather than a stream of drag-history entries.

Dear PyGui uses a viewport draw handle plus mouse down/move/release handlers; Tkinter overlays a native `ttk.Sizegrip`. During motion the host may resize only the disposable live preview item for immediate feedback. Project state changes only on release. Successful preview replacement is rebound by stable ID, stale callbacks are rejected, and undo restores the previous/default geometry while preserving selection. The resize coordinator imports no concrete toolkit and neither host imports designer-edit commands.

The same tranche repairs the remaining constrained-width Tkinter Inspector issue found during Tranche-8 acceptance. Actions are no longer a third horizontal grid column beside the editor. Apply / None / Default occupy a second row beneath the relevant editor, allowing the editor column and action controls to remain usable inside the existing scroll canvas at normal and maximized window sizes.

The first real-Windows automated pass was green at 15 / 15 resize tests, 292 / 292 designer/relocation, 44 / 44 Tkinter and both complete discovery forms at **902 / 902** with one expected skip, but visual acceptance rejected the original Dear PyGui handle behavior before publication. The initial square straddled the selected control edge, letting the surrounding parent/root win selection when its outer half was pressed. The repair keeps the full handle inside the selected item, shares transient pointer arbitration between DPG selection and resize hosts, blocks normal selection for a handle-owned press, and freezes the starting stable ID/native item until release. Three focused regressions raise the repaired gate to **18 / 18**, designer/relocation to **295 / 295** and complete discovery to **905 / 905**; live Tkinter remains **44 / 44**. Real-Windows acceptance independently resized the first and second preview Buttons without promoting either gesture to the root/container, confirmed one-step commit/Undo and the repaired Tkinter Inspector actions, then published the tranche at `1870e053c444e2ca390543cb62a721b162f5ff61` (`Add designer pointer resize surface`).

This tranche deliberately stops at one selected-component bottom-right resize gesture. Hierarchy drag/reparent, palette drag/drop, richer creation templates, multi-selection/mixed values, snapping/alignment guides and multi-handle edge/corner resizing remain later work.

### A34. Component reset and scrubbable numeric manipulation — post-v0.5.1 Tranche 10 full-commit gate

Tranche 10 makes the accepted sizing/property foundation faster to use before hierarchy drag/drop begins. `DesignerInspectorPanel` gains one semantic Reset action that clears every currently explicit property already marked unsettable/Default-capable. Those clears are grouped into one `Reset component to defaults` composite edit, one history entry and one preview rebuild. Properties with no portable Default contract remain unchanged rather than inventing toolkit- or type-specific fallback values.

`app/framework/designer_numeric_drag.py` defines one backend-neutral pointer-delta translation contract. Normal drag is 1x, Shift is coarse 10x and Ctrl is fine 0.1x. Integer/dimension properties round only after applying the total drag displacement, allowing fine motion to accumulate; floating-point properties retain fractional values and existing minimum/maximum metadata clamps candidates. Dear PyGui and Tkinter Inspector hosts expose a small horizontal `<>` scrub affordance for integer/number/dimension editors and commit the final candidate once on release through the same existing property transaction. Default Width/Height starts from rendered preview dimensions so a first scrub behaves relative to what the user sees.

The selected-component resize hosts consume the same modifier sensitivity. Dear PyGui keeps Tranche-9 pointer arbitration and stable drag ownership. Tkinter replaces `ttk.Sizegrip` with a dark ordinary child Frame positioned inside the selected component so the designer gesture cannot resize the outer toplevel window. The compatibility adapter remains secondary, but its behavior must still preserve semantic parity.
...
## 36. v0.5.1 release checkpoint — prepared

The v0.5.1 checkpoint packages the complete backend-neutral designer semantic-core sequence accepted through Tranche 21 without changing SalixTorrent's code-first reference-application role. The release boundary includes the extracted runtime/framework foundations, dual Dear PyGui/Tkinter compatibility proof, designer metadata/editing/history, stable hierarchy and selection state, project-file ownership, transactional preview reconstruction, clipboard operations, property-inspector projection, workspace coordination and shell-command presentation.

`APP_VERSION` and the Inno Setup fallback version are advanced together to `0.5.1`. The complete Windows discovery baseline remains 759 / 759 with one expected non-Windows shell-behavior skip before this release-only metadata/documentation change. The release commit must be accepted with the normal full validator, visual smoke, focused release-packaging regression and `pre_commit_check.bat` before an annotated `v0.5.1` tag is created.

This tag does not freeze the provisional designer project schema, package names or public APIs. Concrete Dear PyGui/Tkinter editor-shell widgets, file-dialog/template policy, paste placement, multi-document ownership, autosave/recovery, pointer hit-testing, visual selection overlays, drag/drop/reparent gestures, resize handles, mixed-value multi-selection and final package/API naming remain post-v0.5.1 work.
