...
**Roadmap status:** v0.5.1 is released/tagged at `8212ac7cc26e4b781d2e5aa0aa3d5981c27022d0`; post-v0.5.1 Tranches 1 through 7 are published and Tranche 8 designer preview sizing/explicit resize controls use the full-commit Windows-acceptance workflow
**Current implementation checkpoint:** post-v0.5.1 Tranche 8 preview sizing/explicit resize controls are the current implementation/docs boundary over published Tranche 7 at `8d1d241538e6beb2849994e4187bc940b1d17f20`; publication is valid only after the real-Windows validator, visual smoke and pre-commit gate pass
**Current real Windows regression baseline:** 871 / 871 in both accepted post-v0.5.1 Tranche-7 discovery forms with one expected non-Windows shell-behavior skip; structural shortcuts 16 / 16, designer/relocation 264 / 264, GUI components 64 / 64 and Tkinter 41 / 41; localization remains 1,337/current, Dear PyGui/Tkinter/SalixTorrent visual smoke and pre-commit passed.
...
Preparation adds 13 focused preview-sizing tests, raises the combined designer/relocation gate to 277 / 277, the live Tkinter gate to 42 / 42 and complete discovery to **885 / 885**. Real-Windows Dear PyGui acceptance must confirm conventional default control sizes at normal/maximized editor dimensions, visible Inspector resize actions, explicit Width/Height changes and Default restoration on the selected component.

A later direct resize handle should measure the selected rendered item only as gesture input, then commit the resulting semantic width/height through these same inspector/workspace contracts. Hierarchy drag/reparent and richer creation templates remain separate later slices.
